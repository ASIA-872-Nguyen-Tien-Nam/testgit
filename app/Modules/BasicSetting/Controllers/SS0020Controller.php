...
class SS0020Controller extends Controller
{
	...
	public function getLeftContent(Request $request)
	{
		...
		if ($validator->passes()) {
			...
			$res = $this->authorityService->searchAuthority($params,0);
			$data['list'] = $res[0] ?? [];
			$data['paging'] = $res[1][0] ?? [];
			// render view
			if ( $request->ajax() ){
				...
				return view('BasicSetting::ss0020.leftcontent', $data);
			}
			else{
				if(isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999'){
	...
	public function getRightContent(Request $request)
	{
		...
		if ($validator->passes()) {
			...
			$res = $this->authorityService->getAuthority($params,0);
			if(isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999'){
				return response()->view('errors.query',[],501);
			}
			$data0 = $res[0] ?? [];
		...
		if ( $request->ajax() )
		{
			try {
				...
				if($this->respon['status'] == OK)
				{
					...
					$result = $this->authorityService->saveAuthority($params,0);
					// check exception
	...
	public function postDelete(Request $request)
	{
		if ( $request->ajax() )
		{
			try {
				...
				$result = $this->authorityService->deleteAuthority($params,0);
				// check exception
				if(isset($result[0][0]) && $result[0][0]['error_typ'] == '999'){
					return response()->view('errors.query',[],501);
				}else if(isset($result[0]) && !empty($result[0])){
