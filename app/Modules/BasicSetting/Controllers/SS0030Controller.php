...
class SS0030Controller extends Controller
{
	...
	public function getIndex(Request $request)
	{
		...
		$data['title'] = trans('messages.bs_authority_assignment_setting');
		...
		//$data['language'] = session_data()->language;
		return view('BasicSetting::ss0030.index', array_merge($data, $post));
	}
	/**
	 * search data
	 * @author tuantv
	...
	public function postSearch(Request $request)
	{
		...
		if ($request->ajax()) {
			...
			return view('BasicSetting::ss0030.search', $data)->render();
		} else {
			if (isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999') {
	...
	public function postSave(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->saveAccountAuthority($params, 0);
				// check exception
	...
	public function postDelete(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->deleteAccountAuthority($params, 0);
				// check exception
				if (isset($result[0][0]) && $result[0][0]['error_typ'] == '999') {
					return response()->view('errors.query', [], 501);
				} else if (isset($result[0]) && !empty($result[0])) {
