...
class ES0020Controller extends Controller
{
	...
	public function index(Request $request)
	{
		...
		return view('EmployeeInfo::es0020.index',array_merge($data, $left))
			...
			->with('lvl4',$res[5] ?? [])
			->with('lvl5',$res[6] ?? [])
			->with('M0022',getCombobox('M0022',1,4)??[])
	...
	public function getLeftContent(Request $request)
	{
		...
		if ($validator->passes()) {
			...
			$res = $this->authorityService->searchAuthority($params,5);
			$data['list'] = $res[0] ?? [];
			$data['paging'] = $res[1][0] ?? [];
			// render view
			if ( $request->ajax() ){
				...
				return view('EmployeeInfo::es0020.leftcontent', $data);
			}
			else{
				if(isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999'){
	...
	public function getRightContent(Request $request)
	{
		...
		if ($validator->passes()) {
			...
			$res = $this->authorityService->getAuthority($params,5);
			if(isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999'){
				return response()->view('errors.query',[],501);
			}
			$data0 = $res[0] ?? [];
		...
		if ( $request->ajax() )
		{
			try {
				...
				if($this->respon['status'] == OK)
				{
					...
					$result = $this->authorityService->saveAuthority($params,5);
					// check exception
	...
	public function postDelete(Request $request)
	{
		if ( $request->ajax() )
		{
			try {
				...
				$result = $this->authorityService->deleteAuthority($params,5);
				// check exception
				if(isset($result[0][0]) && $result[0][0]['error_typ'] == '999'){
					return response()->view('errors.query',[],501);
				}else if(isset($result[0]) && !empty($result[0])){
