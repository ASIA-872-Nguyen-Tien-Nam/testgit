...
class ES0030Controller extends Controller
{
	...
	public function getIndex(Request $request)
	{
		...
		$data['title'] = trans('messages.e_auth_assign_setting');
		$params['company_cd'] = session_data()->company_cd;
		$params['authority_cd'] = session_data()->empinfo_authority_cd;
		...
		$data['combo_organization'] = getCombobox('M0020', 1, 6) ?? [];
		return view('EmployeeInfo::es0030.index', array_merge($data, $post));
	}
	...
	{
		$data = $request->data;
		$belong_cd1 = session_data()->empinfo_authority_typ == 2 ? (session_data()->m0070->belong_cd1 ?? '') : '-1';
		$belong_cd2 = session_data()->empinfo_authority_typ == 2 ? (session_data()->m0070->belong_cd2 ?? '') : '-1';
		$belong_cd3 = session_data()->empinfo_authority_typ == 2 ? (session_data()->m0070->belong_cd3 ?? '') : '-1';
		$belong_cd4 = session_data()->empinfo_authority_typ == 2 ? (session_data()->m0070->belong_cd4 ?? '') : '-1';
		$belong_cd5 = session_data()->empinfo_authority_typ == 2 ? (session_data()->m0070->belong_cd5 ?? '') : '-1';
		...
		$param2['authority_cd'] = session_data()->empinfo_authority_cd;
		$param2['authority_typ'] = session_data()->empinfo_authority_typ;
	
		$data_init = $this->accountAuthorityService->getAccountAuthority($param2, 5);
		if (isset($data_init[0][0]['error_typ']) && $data_init[0][0]['error_typ'] == '999') {
			return response()->view('errors.query', [], 501);
		}
		$res = $this->accountAuthorityService->searchAccountAuthority($params, 5);
		...
		$data['list'] = $res[0] ?? [];
		$data['paging'] = $res[1][0] ?? [];
		$data['authority_typ'] = session_data('empinfo_authority_typ');
		...
		if ($request->ajax()) {
			...
			return view('EmployeeInfo::es0030.search', $data)->render();
		} else {
			if (isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999') {
	...
	public function postSave(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->saveAccountAuthority($params, 5);
				// check exception
	...
	public function postDelete(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->deleteAccountAuthority($params, 5);
				// check exception
				if (isset($result[0][0]) && $result[0][0]['error_typ'] == '999') {
					return response()->view('errors.query', [], 501);
				} else if (isset($result[0]) && !empty($result[0])) {
