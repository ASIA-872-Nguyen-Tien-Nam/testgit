    ...
    <div class="col-md-12 col-xs-12 col-lg-12">
        <div class="list-search-v">
            ...
            <div class="list-search-content">
                @if (isset($list[0]))
                    @foreach ($list as $row)
                        <div class="text-overfollow list-search-child" id="{{ $row['qualification_cd'] }}" data-container="body"
                            data-toggle="tooltip" data-original-title="{{ $row['qualification_nm'] }}">{{ $row['qualification_nm'] }}</div>
                    @endforeach
                @else
