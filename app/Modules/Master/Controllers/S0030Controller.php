...
class S0030Controller extends Controller
{
	...
	public function getIndex(Request $request)
	{
		...
		return view('Master::s0030.index', array_merge($data, $post));
	}
	/**
	 * search data
	 * @author tuantv
	...
	public function postSearch(Request $request)
	{
		...
		$data_init = $this->accountAuthorityService->getAccountAuthority($param2, 1);
		//
		// $res = Dao::executeSql('SPC_S0030_LST1', $params);
		$res = $this->accountAuthorityService->searchAccountAuthority($params, 1);
		//
		...
		if ($request->ajax()) {
			...
			return view('Master::s0030.search', $data)->render();
		} else {
	...
	public function postSave(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->saveAccountAuthority($params, 1);
				// check exception
	...
	public function postDelete(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->deleteAccountAuthority($params, 1);
				// check exception
				if (isset($result[0][0]) && $result[0][0]['error_typ'] == '999') {
					$this->respon['status']     = EX;
					$this->respon['Exception']  = $result[0][0]['remark'];
