	...
	<div class="col-md-12 col-xs-12 col-lg-12">
		<div class="list-search-v">
			...
			<div class="list-search-content" >
				@if ( isset($list[0]) )
					@foreach($list as $row)
			<div class="text-overfollow list-search-child  {{$row['company_cd'] == ($company_refer??0)?'active':''}}" id="{{ $row['company_cd'] }}"  data-container="body" data-toggle="tooltip" data-original-title="{{$row['company_nm']}}">{{$row['company_nm']}}</div>
					@endforeach
				@else
