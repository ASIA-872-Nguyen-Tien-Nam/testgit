...
class OQ2031Controller extends Controller
{
      ...
      * @created at 2020-09-04 08:33:47
     ...
     public function index(Request $request)
     {
          ...
          $data['title'] = trans('messages.analysis_sufficiency');
          ...
          return view('OneOnOne::oq2031.index', $data);
     }
     ...
     public function postSearch(Request $request)
     {
          ...
          $params['mode']               =  0;
          $result = $this->analysisService->getFulfillmentResult($params);
          if (isset($result[0][0]['error_typ']) && $result[0][0]['error_typ'] == '999') {
               return response()->view('errors.query', [], 501);
          }
          $data['list']                 =  $result[0] ?? [];
          $data['header']               =  $result[1] ?? [];
          $data['target_typ']           =  $result[0][0]['target_typ'] ?? 0;     // 0.全社 1.部署 2.職種 3.等級 4.年齢
          // render view
          if ($request->ajax()) {
               return view('OneOnOne::oq2031.search', $data);
          } else {
               return $data;
          }
     }
     ...
     public function postExportExcel(Request $request)
     {
          if ($request->ajax()) {
               ...
               try {
                    ...
                    //
                    $store_name = 'SPC_oQ2031_FND1';
                    $typeReport = 'FNC_OUT_EXL';
                    $screen = 'OQ2031';
                    $file_name = 'OQ2031_' . time() . '.xlsx';
