...
class OS0030Controller extends Controller
{
	...
	public function getIndex(Request $request)
	{
		...
		$data['title'] = trans('messages.os0030');
		...
		$data['organization_group'] = getCombobox('M0022', 1, 2) ?? [];
		$data['combo_organization'] = getCombobox('M0020', 1, 2) ?? [];
		return view('OneOnOne::os0030.index', array_merge($data, $post));
	}
	/**
	 * search data
	 * @author tuantv
	...
	public function postSearch(Request $request)
	{
		...
		$data_init = $this->accountAuthorityService->getAccountAuthority($param2, 2);
		if (isset($data_init[0][0]['error_typ']) && $data_init[0][0]['error_typ'] == '999') {
			return response()->view('errors.query', [], 501);
		}
		//
		$res = $this->accountAuthorityService->searchAccountAuthority($params, 2);
		...
		if ($request->ajax()) {
			...
			return view('OneOnOne::os0030.search', $data)->render();
		} else {
			if (isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999') {
	...
	public function postSave(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->saveAccountAuthority($params, 2);
				// check exception
	...
	public function postDelete(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->deleteAccountAuthority($params, 2);
				// check exception
				if (isset($result[0][0]) && $result[0][0]['error_typ'] == '999') {
					return response()->view('errors.query', [], 501);
				} else if (isset($result[0]) && !empty($result[0])) {
