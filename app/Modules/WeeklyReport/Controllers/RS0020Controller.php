...
class RS0020Controller extends Controller
{
	...
	public function getLeftContent(Request $request)
	{
		...
		$res = $this->authorityService->searchAuthority($params, 4);
		$data['list'] = $res[0] ?? [];
		$data['paging'] = $res[1][0] ?? [];
		// render view
		if ($request->ajax()) {
			...
			return view('WeeklyReport::rs0020.leftcontent', $data);
		} else {
			if (isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999') {
				return array('error_typ' => '999');
			}
	...
	public function getRightContent(Request $request)
	{
		...
		$res = $this->authorityService->getAuthority($params, 4);
		if (isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999') {
			return response()->view('errors.query', [], 501);
		}
		$data0 	= $res[0] ?? [];
	...
	public function postSave(Request $request)
	{
		try {
			...
			if ($this->respon['status'] == OK) {
				...
				$params['company_cd']   	=   session_data()->company_cd;
				$result = $this->authorityService->saveAuthority($params, 4);
				// check exception
	...
	public function postDelete(Request $request)
	{
		try {
			...
			$params['company_cd']   			=   session_data()->company_cd;
			//
			$result = $this->authorityService->deleteAuthority($params, 4);
			// check exception
			if (isset($result[0][0]) && $result[0][0]['error_typ'] == '999') {
				return response()->view('errors.query', [], 501);
			} else if (isset($result[0]) && !empty($result[0])) {
