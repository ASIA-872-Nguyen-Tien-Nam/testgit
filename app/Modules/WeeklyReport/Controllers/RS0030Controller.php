...
class RS0030Controller extends Controller
{
	...
	public function getIndex(Request $request)
	{
		...
		$data['title'] = trans('messages.authority_assignment');
		...
		return view('WeeklyReport::rs0030.index', array_merge($data, $post));
	}
	/**
	 * search data
	 * @author tuantv
	...
	{
		$data = $request->data;
		$belong_cd1 = session_data()->report_authority_typ == 2 ? (session_data()->m0070->belong_cd1 ?? '') : '-1';
		$belong_cd2 = session_data()->report_authority_typ == 2 ? (session_data()->m0070->belong_cd2 ?? '') : '-1';
		$belong_cd3 = session_data()->report_authority_typ == 2 ? (session_data()->m0070->belong_cd3 ?? '') : '-1';
		$belong_cd4 = session_data()->report_authority_typ == 2 ? (session_data()->m0070->belong_cd4 ?? '') : '-1';
		$belong_cd5 = session_data()->report_authority_typ == 2 ? (session_data()->m0070->belong_cd5 ?? '') : '-1';
		...
		$param2['authority_cd'] = session_data()->report_authority_cd??'';
		$param2['authority_typ'] = session_data()->report_authority_typ??'';
		$data_init = $this->accountAuthorityService->getAccountAuthority($param2, 4);
		if (isset($data_init[0][0]['error_typ']) && $data_init[0][0]['error_typ'] == '999') {
			return response()->view('errors.query', [], 501);
		}
		//
		$res = $this->accountAuthorityService->searchAccountAuthority($params, 4);
		...
		$data['list'] = $res[0] ?? [];
		$data['paging'] = $res[1][0] ?? [];
		$data['authority_typ'] = session_data('report_authority_typ');
		...
		if ($request->ajax()) {
			...
			return view('WeeklyReport::rs0030.search', $data)->render();
		} else {
			if (isset($res[0][0]['error_typ']) && $res[0][0]['error_typ'] == '999') {
	...
	public function postSave(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->saveAccountAuthority($params, 4);
				// check exception
	...
	public function postDelete(Request $request)
	{
		if ($request->ajax()) {
			try {
				...
				$result = $this->accountAuthorityService->deleteAccountAuthority($params, 4);
				// check exception
				if (isset($result[0][0]) && $result[0][0]['error_typ'] == '999') {
					return response()->view('errors.query', [], 501);
				} else if (isset($result[0]) && !empty($result[0])) {
