	...
	<div class="col-md-12 col-xs-12 col-lg-12">
		<div class="list-search-v">
			...
			<div class="list-search-content">
                @if (isset($list[0]) )
                    @foreach($list as $row)
                        <div class="text-overfollow list-search-child"  data-container="body" data-toggle="tooltip" data-original-title="{{$row['name']}}">
                            {{$row['name']}}
							<input type="hidden" class="mark_kbn_cd" value="{{$row['mark_kbn']}}">
                        </div>
                    @endforeach
                @else
