    ...
    <div class="col-md-12 col-xs-12 col-lg-12">
        <div class="list-search-v">
            ...
            <div class="list-search-content">
                @if (isset($list[0]))
                    @foreach ($list as $row)
                        <div class="text-overfollow list-search-child" data-container="body" data-toggle="tooltip"
                            data-original-title="{{ $row['mygroup_nm']??''}}">
                            {{ $row['mygroup_nm']??'' }}
                            <input type="hidden" class="mygroup_cd" value="{{$row['mygroup_cd']??''}}">
                            <input type="hidden" class="mygroup_nm" value="{{$row['mygroup_nm']??''}}">
                        </div>
                    @endforeach
                @else
