DROP PROCEDURE [SPC_I2040_RPT1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--EXEC SPC_I2040_RPT1 '{"fiscal_year":"2021","treat1":"-1","treat2":"-1","treat3":"-1","employee_typ":"-1","employee_cd":"","rater":"","rank":"-1","list":[],"list_treatment_applications_no":[{"treatment_applications_no":"1"}],"list_organization_step1":[],"list_organization_step2":[],"list_organization_step3":[],"list_organization_step4":[],"list_organization_step5":[],"list_position_cd":[],"list_grade":[],"list_evalute_step":[],"list_character":[{"character_item":"","item_cd":"1"}],"list_date":[{"from_date_item":"","to_date_item":"","item_cd":"3"}],"list_number_item":[{"from_number_item":"","to_number_item":"","number_item":"","item_cd":"2"},{"number_item":"","item_cd":"5"}]}','10000','hanhntm';
...
--*　更新者/updater				:　 viettd
--*　更新内容/update content		:	FIX BUG NOT VIEW STEP 2
--*   					
...
--*   					
--*  更新日/update date			:	2021/09/16
--*　更新者/updater				:	viettd　
--*　更新内容/update content		:	fix not show (最終評価)評語
--*   					
...
--*
--*  更新日/update date			:	2022/11/29
--*　更新者/updater				:	namnt
--*　更新内容/update content		:	add english label
--*   					
...
--*
--****************************************************************************************
CREATE PROCEDURE [SPC_I2040_RPT1]
	@P_json						nvarchar(max)	=	''
...
BEGIN
	...
	,	@pageNumber							int				=	0
	,	@i									int				=	1
	,	@w_max_count_rater					int				=	0
	,	@w_min_count_rater					int				=	0
	,	@count_rater						int				=	0
	,	@year_month_day						date			=	NULL
	,	@login_position_cd					INT				=	0
	,	@arrange_order						int				=	0
	,	@use_typ							INT				=	0
	,	@belong_cd1							NVARCHAR(20)	=	0
	,	@belong_cd2							NVARCHAR(20)	=	0
	,	@beginning_date						date			=	NULL
	,	@current_year_1						date			=	SYSDATETIME()
	,	@current_year						int				=	DATEPART(YYYY,GETDATE())
	,	@max_confirm_last_step				INT				=	0
	...
	,	@str_lbl							NVARCHAR(MAX)	=	''
	,	@screen_rater						nvarchar(20)	=	''
	,	@max_feedback_status				INT				=	0
	...
	,	column_nm					NVARCHAR(200)
	,	column_point_sum			NVARCHAR(200)
	,	rank_of_sheet				INT
	)
	...
	,	rank_kinds					int
	,	rank_nm						nvarchar(20)
	,	confirm_datetime			datetime			-- add by viettd 2021/04/07
	)
	...
	--#TEMP_F0120_WHILE for pivot get sheet_nm, point sum,weight
	CREATE TABLE #TEMP_F0120_WHILE(
		id							int		identity(1,1)
	,	company_cd					int
	,	fiscal_year					int
	,	employee_cd					nvarchar(20)
	...
	,	item_nm					nvarchar(20)
	,	col_nm					nvarchar(20)
	,	item_order				nvarchar(20)
	)
	-- #TBL_PointSum_Pivot
	CREATE TABLE #TBL_PointSum_Pivot(
		id							INT			IDENTITY(1,1)
	,	employee_cd					NVARCHAR(50) 
	,	employee_nm					NVARCHAR(200) 
	,	employee_typ_nm				NVARCHAR(50) 
	...
	,	position_nm					NVARCHAR(50)
	,	grade_nm					NVARCHAR(50)
	,	point_sum1					NVARCHAR(50)
	,	adjust_point1				NVARCHAR(50)
	,	rank_kinds1					NVARCHAR(50)
	,	point_sum2					NVARCHAR(50)
	,	adjust_point2				NVARCHAR(50)
	,	rank_kinds2					NVARCHAR(50)
	,	point_sum3					NVARCHAR(50)
	,	adjust_point3				NVARCHAR(50)
	,	rank_kinds3					NVARCHAR(50)
	,	point_sum4					NVARCHAR(50)
	,	adjust_point4				NVARCHAR(50)
	,	rank_kinds4					NVARCHAR(50)
	,	point_sum5					NVARCHAR(50)
	,	rank_kinds5					NVARCHAR(50)
	,	final_evaluation_can_edited	NVARCHAR(50)				-- 0.not view 1.view  2.can edited
	,	comment						NVARCHAR(800)
	,	last_comment				NVARCHAR(800)
	,	treatment_applications_no	NVARCHAR(50)
	,	treatment_applications_nm	NVARCHAR(50)
	,	rank_prev_1_nm				NVARCHAR(50)
	,	rank_prev_2_nm				NVARCHAR(50)
	,	rank_prev_3_nm				NVARCHAR(50)
	,	rater_status				NVARCHAR(100)
	,	confirm_last_step			NVARCHAR(100)
	,	m0100_step					NVARCHAR(100)
	,	max_confirm_last_step		NVARCHAR(100)
	,	feed_back_status			NVARCHAR(100)
	,	final_rater_status			NVARCHAR(100)
	)
	--#TBL_HEADER_DYNAMIC
	CREATE TABLE #TBL_HEADER_DYNAMIC(
		ID							BIGINT	IDENTITY(1,1)
	,	col_temp_name				NVARCHAR(20) 
	,	col_temp					NVARCHAR(100) 
	)
	...
	--
	INSERT INTO #NAME_COLUMN VALUES
	...
	-- ↓↓↓ add by vietd 2021/04/02
	-- if admin & @authority_typ < 2 then set admin to rater
	IF @authority_typ = 3 AND @authority = 0
	BEGIN
		SET @authority_typ = 2 -- 2.RATER
	END
	-- ↑↑↑ end add by vietd 2021/04/02
	
	IF @authority_typ = 6
	BEGIN
		SET @authority_typ = 2 -- 2.RATER
	END
	-- ↑↑↑ end add by vietd 2022/08/16
	...
	BEGIN
		INSERT INTO #TABLE_LABEL
		SELECT 
			S0011.item_cd
		,	CASE 	WHEN  S0011.item_cd >= 4 AND S0011.item_cd <= 8 AND M0022.use_typ =1 THEN M0022.organization_group_nm
					WHEN S0011.item_cd = 12 THEN		CAST(@fiscal_year - 3 AS NVARCHAR(20)) + '年度' 
					WHEN S0011.item_cd = 13 THEN		CAST(@fiscal_year - 2 AS NVARCHAR(20)) + '年度' 
					WHEN S0011.item_cd = 14 THEN		CAST(@fiscal_year - 1 AS NVARCHAR(20)) + '年度' 
					ELSE L0031.item_nm
			END AS item_nm
		,	#NAME_COLUMN.col_nm
		,	#NAME_COLUMN.item_order
		FROM S0011 LEFT JOIN L0031 ON(
			S0011.item_cd = L0031.item_cd
		AND L0031.del_datetime IS NULL
		)
		LEFT JOIN #NAME_COLUMN ON(
			S0011.item_cd = #NAME_COLUMN.item_cd
		)
		LEFT JOIN M0022 ON(
		...
		AND (
					S0011.item_cd < 4 OR S0011.item_cd > 8 OR	M0022.use_typ = 1 OR M0022.company_cd IS NULL
			)
	...
	BEGIN
		...
		SELECT 
			L0031.item_cd
		,	CASE 	WHEN  L0031.item_cd >= 4 AND L0031.item_cd <= 8 AND M0022.use_typ =1 THEN M0022.organization_group_nm
								WHEN L0031.item_cd = 12 THEN		CAST(@fiscal_year - 3 AS NVARCHAR(20)) + '年度' 
								WHEN L0031.item_cd = 13 THEN		CAST(@fiscal_year - 2 AS NVARCHAR(20)) + '年度' 
								WHEN L0031.item_cd = 14 THEN		CAST(@fiscal_year - 1 AS NVARCHAR(20)) + '年度' 
								ELSE L0031.item_nm
								END AS item_nm
		,	#NAME_COLUMN.col_nm
		,	#NAME_COLUMN.item_order
	...
	,	M0102.treatment_applications_nm										--treatment_applications_nm	
	--↓↓↓ edited by viettd 2021/07/28
	,	CASE
	...
	LEFT JOIN M0050 ON (
		#M0070H.company_cd		=	M0050.company_cd
	AND	#M0070H.grade			=	M0050.grade
	...
	LEFT JOIN M0050 AS M0050_SHEET ON (
		#M0071_SHEET.company_cd		=	M0050_SHEET.company_cd
	AND	#M0071_SHEET.grade			=	M0050_SHEET.grade
	AND	M0050_SHEET.del_datetime IS NULL
	)
	LEFT JOIN M0100 ON(
	...
	UPDATE #TABLE_RESULT SET 
		rater_status =	CASE 
							WHEN #TABLE_RESULT.rater_status = 0 AND #TABLE_RESULT.current_rater_step <> 0	-- edited by viettd 2021/04/07  
							THEN current_rater_step 
	...
	UPDATE #TABLE_RESULT SET 
		final_rater_status		=	CASE 
										WHEN @authority_typ	IN (3,4,5)  AND current_rater_step = 5
										THEN 5
	...
	UPDATE #TABLE_RESULT SET
		accept_input =	CASE 
							WHEN  (#F0120.employee_cd IS  NULL  AND #TABLE_RESULT.sheet_use_typ = 1) OR M0100_rater_check_typ = 0 THEN 1
							ELSE 0
						END	 
	FROM #TABLE_RESULT
	LEFT JOIN(
		...
		,	W_M0200.sheet_kbn
		,	CASE WHEN W_M0200.sheet_kbn = 1 AND  (	
		...
		FROM #TABLE_RESULT LEFT JOIN #F0032 ON(
	...
	,	CASE WHEN  F0100.rater_employee_cd_4 <> '' 	THEN 4
					WHEN  F0100.rater_employee_cd_4 = '' AND  F0100.rater_employee_cd_3 <> ''	THEN 3
					WHEN  F0100.rater_employee_cd_4 = '' AND  F0100.rater_employee_cd_3 = '' AND  F0100.rater_employee_cd_2 <> ''	THEN 2
					WHEN  F0100.rater_employee_cd_4 = '' AND  F0100.rater_employee_cd_3 = '' AND  F0100.rater_employee_cd_2 = '' AND 	F0100.rater_employee_cd_1 <> ''THEN 1  
					ELSE 0
		END
	FROM #TABLE_RESULT 
	LEFT JOIN #F0032 ON (
	...
	,	RANK () OVER ( PARTITION BY #F0032.fiscal_year,#F0032.employee_cd,#F0032.treatment_applications_no
			ORDER BY M0200.sheet_kbn,#F0032.sheet_cd ASC
		)  
	...
	WHERE #F0032.company_cd	=	@P_company_cd
	AND	(#F0032.fiscal_year	=	@fiscal_year )
	GROUP BY 
	#F0032.company_cd,
	#F0032.fiscal_year,
	...
	M0200.sheet_nm
	--■■■■■■■■■■■■■■■■■■■■■■■■■■■■■CACULATE POINT SUM ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
	--- CHIA RA 2 TH 1.N?U CO B?N GHI T? F0201 THI S? L?Y TH?NG DAT T? F0201
	---ELSE	S? L?Y T? F0200 JOIN VS F0201
	UPDATE #TABLE_RESULT SET
		data_f0201_status = 1
	FROM F0201 LEFT JOIN #TABLE_RESULT ON(
										F0201.company_cd				=	@P_company_cd
									...
									WHERE F0201.company_cd				=	@P_company_cd
									AND	 F0201.fiscal_year				=	@fiscal_year	
									AND	F0201.treatment_applications_no	=	#TABLE_RESULT.treatment_applications_no
									AND	 F0201.del_datetime	IS NULL
	...
	,	MAX(F0201.confirm_datetime)	   AS confirm_datetime			-- add by viettd 2021/04/07
	FROM F0201
	...
	WHERE F0201.del_datetime IS NULL
	...
	,	MAX(W_M0130.rank_nm)				AS rank_nm
	,	NULL								AS confirm_datetime			-- add by viettd 2021/04/07
	FROM F0200
	...
	,	MAX(F0200.rank_cd)																				AS rank_cd
	,	MAX(W_M0130.rank_nm)																			AS rank_nm
	FROM  #TEMP_F0120 INNER JOIN F0120 ON(
	...
	AND F0200.del_datetime IS NULL
	AND F0200.rank_cd							<>	0		-- add by viettd 2021/04/07
	)
	LEFT JOIN #F0032 on(
		F0120.company_cd						=	#F0032.company_cd
	AND	F0120.fiscal_year						=	#F0032.fiscal_year
	...
	AND	(
			ISNULL(#TMP_POINT_SUM.point_sum,0) >= ISNULL(W_M0130.points_from,0) 
		AND ISNULL(#TMP_POINT_SUM.point_sum,0) < IIF(W_M0130.points_to IS NULL,999999,W_M0130.points_to)
		)
	AND W_M0130.fiscal_year = @fiscal_year
	...
	AND	(#TMP_POINT_SUM.evaluation_step				= #STEP_LOGIN.evaluation_step OR  @authority_typ IN(3,4,5))
	)
	WHERE #STEP_LOGIN.employee_cd IS NOT NULL --AND @fiscal_year = @current_year
	...
	AND	(
			ISNULL(#TMP_POINT_SUM_FINAL_最終.point_sum,0) >= ISNULL(W_M0130.points_from,0) 
		AND ISNULL(#TMP_POINT_SUM_FINAL_最終.point_sum,0) < IIF(W_M0130.points_to IS NULL,999999,W_M0130.points_to)
		)
	AND W_M0130.fiscal_year = @fiscal_year
	AND W_m0130.treatment_applications_no = #TMP_POINT_SUM_FINAL_最終.treatment_applications_no
	AND W_M0130.del_datetime IS NULL
	)
	-- add by viettd 2021/09/16
	-- IF rank_nm NOT EIXTS IN W_M0130 THEN GET FROM M0130
	UPDATE #TMP_POINT_SUM_FINAL_最終 SET
		rank_nm = ISNULL(M0130.rank_nm,'')
	FROM #TMP_POINT_SUM_FINAL_最終 AS S
	INNER JOIN M0130 ON (
		S.company_cd				=	M0130.company_cd
	AND S.treatment_applications_no	=	M0130.treatment_applications_no
	AND S.rank_kinds				=	M0130.rank_cd
	AND M0130.del_datetime IS NULL
	)
	WHERE
		S.rank_kinds	>	0
	AND ISNULL(S.rank_nm,'') = ''
	-- #TMP_POINT_SUM
	UPDATE #TMP_POINT_SUM SET
		rank_nm = ISNULL(M0130.rank_nm,'')
	FROM #TMP_POINT_SUM AS S
	INNER JOIN M0130 ON (
		S.company_cd				=	M0130.company_cd
	AND S.treatment_applications_no	=	M0130.treatment_applications_no
	AND S.rank_kinds				=	M0130.rank_cd
	...
	WHERE
		S.rank_kinds	>	0
	AND ISNULL(S.rank_nm,'') = ''
	-- end add by viettd 2021/09/16
	-- UPDATE #TABLE_RESULT feed_back_status
	UPDATE #TABLE_RESULT SET 
	...
	-- UPDATE confirm_last_step_final TO #TABLE_RESULT
	UPDATE #TABLE_RESULT SET
	...
	--
	SET @max_feedback_status = (SELECT MAX(feed_back_status) FROM #TABLE_RESULT)
	--
	IF EXISTS (SELECT 1 FROM #TABLE_RESULT WHERE rater_status = 0 AND final_rater_status = 5)
	BEGIN 
		UPDATE #TABLE_RESULT SET rater_step = 5
	END
	--
	UPDATE #TABLE_RESULT SET
	...
	AND	#TABLE_RESULT.treatment_applications_no	=	F0200.treatment_applications_no
	AND		(
				#STEP_LOGIN.evaluation_step		=	F0200.evaluation_step
	...
	--
	UPDATE #TABLE_RESULT SET
		@max_confirm_last_step		= (select	MAX(confirm_last_step)
	FROM #TABLE_RESULT )
	--
	UPDATE #TABLE_RESULT SET
	...
	-- set @count_rater
	SELECT 
		@w_max_count_rater	=	MAX(TEMP.rater_status)
	,	@w_min_count_rater	=	MIN(TEMP.rater_status)
	FROM #TABLE_RESULT	AS TEMP 
	--
	IF	@w_min_count_rater	=	0 OR @authority_typ in(3,4,5) OR @max_feedback_status = 1
	BEGIN
		SET	@count_rater	=	6	-- view all 
	END
	ELSE
	BEGIN
		SET	@count_rater	=	@w_max_count_rater + 2
	END
	...
	FROM #TEMP_F0120 
	ORDER BY #TEMP_F0120.sheet_cd
	--
	WHILE @i <=	6
	BEGIN
		INSERT INTO #TEMP_F0120_WHILE
		SELECT 
			#TEMP_F0120.company_cd
		,	#TEMP_F0120.fiscal_year
		,	#TEMP_F0120.employee_cd
		,	#TEMP_F0120.sheet_cd
		,	#TEMP_F0120.treatment_applications_no
		,	#TEMP_F0120.sheet_nm
		,	CASE
				WHEN	@i	=	1
				THEN	CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'A'
				WHEN	@i	=	2
				THEN	CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'B'
				WHEN	@i	=	3
				THEN	CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'C'
				WHEN	@i	=	4
				THEN	CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'D'
				WHEN	@i	=	5
				THEN	CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'E'
				WHEN	@i	=	6 -- sheet_cd to get sheet_nm
				THEN	CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)
				ELSE	''
			END		
		,	CASE
				WHEN	@i	=	1 AND ((feed_back_status = 1 AND rater_status = 1) OR rater_status >= 1)
				THEN	CAST(ISNULL(STEP1.point_sum,0) AS NVARCHAR(10))
				WHEN	@i	=	2 AND ((feed_back_status = 1 AND rater_status = 1) OR rater_status >= 2)
				THEN	CAST(ISNULL(STEP2.point_sum,0) AS NVARCHAR(10))
				WHEN	@i	=	3 AND ((feed_back_status = 1 AND rater_status = 1) OR rater_status >= 3)
				THEN	CAST(ISNULL(STEP3.point_sum,0) AS NVARCHAR(10))
				WHEN	@i	=	4 AND ((feed_back_status = 1 AND rater_status = 1) OR rater_status >= 4)
				THEN	CAST(ISNULL(STEP4.point_sum,0) AS NVARCHAR(10))
				WHEN	@i	=	5 
				THEN	ISNULL(CAST(#F0032.weight AS nvarchar(10)),'')
				WHEN	@i	=	6 
				THEN	#TEMP_F0120.sheet_nm
				ELSE	''
			END
	FROM #TEMP_F0120	
	LEFT JOIN #F0032 on(
	...
	END
	--GET @count_rater
	IF	@count_rater	=	3 OR(@count_rater	=	6 AND @step = 1)
	BEGIN	
	SELECT @string_column = stuff((select ', ['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'] ,'+ '['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'A] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'E]'
								from (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120
								for xml path('')),1,1,'')
	END
	IF	@count_rater	=	4 OR(@count_rater	=	6 AND @step = 2)
	BEGIN	
	SELECT @string_column = stuff((select ', ['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'] ,'+ '['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'A] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'B] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'E]'
								from (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120
								for xml path('')),1,1,'')
	END
	IF	@count_rater	=	5 OR(@count_rater	=	6 AND @step = 3)
	BEGIN	
	SELECT @string_column = stuff((select ', ['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'] ,'+ '['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'A] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'B] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'C] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'E]'
								from (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120
								for xml path('')),1,1,'')
	END
	IF	@count_rater	=	6 AND @step = 4
	BEGIN	
	SELECT @string_column = stuff((select ', ['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'] ,'+ '['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'A] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'B] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'C] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'D] ,'+'['+ cast((#TEMP_F0120.rank_of_sheet) as nvarchar(10))+'E]'
								from (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120
								for xml path('')),1,1,'')
	END
	-- GET cloumn in s0011 format into string
	SET @str_lbl = (SELECT   col_nm +','
					  FROM #TABLE_LABEL
					  FOR XML PATH ('')
					)
					--print @str_lbl
	...
	BEGIN
		SET @str_lbl = ''
	END
	-- if not eixist #TABLE_RESULT
	IF EXISTS(SELECT 1 FROM #TABLE_RESULT)
	BEGIN
		-- CHECK @string_column
		IF @string_column IS NOT NULL
		BEGIN
			INSERT INTO #TBL_HEADER_DYNAMIC	
			SELECT 
				CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)		
			,	CASE
				WHEN @w_language != 2 THEN '評価要素'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_評価シート'
				ELSE
				'Rating Factor'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_Evaluation Sheet'
				END
			FROM (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120
			INSERT INTO #TBL_HEADER_DYNAMIC	
			SELECT 
				CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'A'
			,	CASE 
				WHEN @w_language != 2 THEN '評価要素'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_一次評価'
				ELSE
				'Rating Factor'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_First Evaluation'
				END
			FROM (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120 
			INSERT INTO #TBL_HEADER_DYNAMIC	
			SELECT 
				CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'B'	
			,	CASE 
				WHEN @w_language != 2 THEN '評価要素'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_二次評価	'	
				ELSE
				'Rating Factor'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_Second Evaluation'
				END
			FROM (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120 
			INSERT INTO #TBL_HEADER_DYNAMIC	
			SELECT 
				CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'C'		
			,	CASE 
				WHEN @w_language != 2 THEN '評価要素'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_三次評価'	
				ELSE
				'Rating Factor'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_Third Evaluation'
				END
			FROM (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120 	
			INSERT INTO #TBL_HEADER_DYNAMIC	
			SELECT 
				CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'D'		
			,	CASE 
				WHEN @w_language != 2 THEN '評価要素'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_四次評価'	
				ELSE
				'Rating Factor'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_Fourth Evaluation'
				END
			FROM (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120
			INSERT INTO #TBL_HEADER_DYNAMIC	
			SELECT 
				CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'E'
			,	CASE 
				WHEN @w_language != 2 THEN '評価要素'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_ウェイト'	
				ELSE
				'Rating Factor'+CONVERT(NVARCHAR(5),#TEMP_F0120.rank_of_sheet)+'_Weight'
				END
			FROM (SELECT DISTINCT rank_of_sheet FROM #TEMP_F0120) AS #TEMP_F0120  
			--
			SET	@_strColumn	=	','+@string_column
			--
			SET @_strExec = REPLACE(@_strColumn,',',';ALTER TABLE #TBL_PointSum_Pivot ADD ') 
			SET @_strExec = STUFF(@_strExec,1,1,'')
			SET @_strExec = REPLACE(@_strExec,';',' NVARCHAR(MAX);' + CHAR(13)) + ' NVARCHAR(MAX);'
			EXECUTE(@_strExec)
			-- SET @_strExec
			IF @w_language = 2
			BEGIN
			SET @_strExec = 
			'INSERT INTO #TBL_PointSum_Pivot
			SELECT 
			''Employee Number''							AS	employee_cd																
			,''Employee Name''							AS	employee_nm													
			,''Employee Classification''						AS	employee_typ_nm																																		
			,''Department１''							AS	belong1_nm																																							
			,''Department２''							AS	belong2_nm	
			,''Department３''							AS	belong3_nm	
			,''Department４''							AS	belong4_nm													
			,''Department５''							AS	belong5_nm													
			,''Job Title''							AS	job_nm														
			,''Position''							AS	position_nm													
			,''Rank''							AS	grade_nm													
			,''(First Evaluation)Evaluation Pts''					AS	point_sum1													
			,''(First Evaluation)Evaluation Range''					AS	adjust_point1												
			,''(First Evaluation)Remarks''					AS	rank_kinds1													
			,''(Second Evaluation)Evaluation Pts''					AS	point_sum2													
			,''(Second Evaluation)Evaluation Range''					AS	adjust_point2												
			,''(Second Evaluation)Remarks''					AS	rank_kinds2													
			,''(Third Evaluation)Evaluation Pts''					AS	point_sum3													
			,''(Third Evaluation)Evaluation Range''					AS	adjust_point3												
			,''(Third Evaluation)Remarks''					AS	rank_kinds3													
			,''(Fourth Evaluation)Evaluation Pts''					AS	point_sum4													
			,''(Fourth Evaluation)Evaluation Range''					AS	adjust_point4												
			,''(Fourth Evaluation)Remarks''					AS	rank_kinds4													
			,''(Final Evaluation)Evaluation Pts''					AS	point_sum5													
			,''(Final Evaluation)Remarks''					AS	rank_kinds5			
			,2									AS	final_evaluation_can_edited										
			,''Comment''						AS	comment
			,	''''							AS	last_comment						
			,''Company Action Code''					AS	treatment_applications_no	
			,''Company Action''						AS	treatment_applications_nm
			,'''+CAST(@fiscal_year - 1 AS NVARCHAR(20)) +'''		AS  rank_prev_1_nm
			,'''+CAST(@fiscal_year - 2 AS NVARCHAR(20)) +'''		AS  rank_prev_2_nm
			,'''+CAST(@fiscal_year - 3 AS NVARCHAR(20)) +'''		AS  rank_prev_3_nm
			,	'+CONVERT(nvarchar(10),(@count_rater-2))+'
			,	0
			,	'+CONVERT(nvarchar(10),@step)+'
			,	'+CONVERT(nvarchar(10),@max_confirm_last_step)+'
			,	0	AS feed_back_status
			,	5	AS final_rater_status
			'+@_strColumn+'
			FROM (
			SELECT 
				col_temp
			,	col_temp_name
			FROM #TBL_HEADER_DYNAMIC
			)
			AS P
			Pivot(MAX(col_temp) FOR col_temp_name IN ('+@string_column+')) AS A
			'
			END
			ELSE
			BEGIN
			...
			,''役職''							AS	position_nm													
			,''等級''							AS	grade_nm													
			,''(一次評価)評価点''					AS	point_sum1													
			,''(一次評価)加減点''					AS	adjust_point1												
			,''(一次評価)評語''					AS	rank_kinds1													
			,''(二次評価)評価点''					AS	point_sum2													
			,''(二次評価)加減点''					AS	adjust_point2												
			,''(二次評価)評語''					AS	rank_kinds2													
			,''(三次評価)評価点''					AS	point_sum3													
			,''(三次評価)加減点''					AS	adjust_point3												
			,''(三次評価)評語''					AS	rank_kinds3													
			,''(四次評価)評価点''					AS	point_sum4													
			,''(四次評価)加減点''					AS	adjust_point4												
			,''(四次評価)評語''					AS	rank_kinds4													
			,''(最終評価)評価点''					AS	point_sum5													
			,''(最終評価)評語''					AS	rank_kinds5			
			,2									AS	final_evaluation_can_edited										
			,''コメント''						AS	comment
			,	''''							AS	last_comment						
			,''処遇用途コード''					AS	treatment_applications_no	
			,''処遇用途''						AS	treatment_applications_nm
			,'''+CAST(@fiscal_year - 1 AS NVARCHAR(20)) +'年度''		AS  rank_prev_1_nm
			,'''+CAST(@fiscal_year - 2 AS NVARCHAR(20)) +'年度''		AS  rank_prev_2_nm
			,'''+CAST(@fiscal_year - 3 AS NVARCHAR(20)) +'年度''		AS  rank_prev_3_nm
			,	'+CONVERT(nvarchar(10),(@count_rater-2))+'
			,	0
			,	'+CONVERT(nvarchar(10),@step)+'
			,	'+CONVERT(nvarchar(10),@max_confirm_last_step)+'
			...
			,	REPLACE(comment,char(10),space(0))	 AS 	comment	
			,	REPLACE(last_comment	,char(10),space(0))	 AS 	last_comment				
			...
			,	#TEMP_F0120_WHILE.column_nm									
			,	#TEMP_F0120_WHILE.column_point_sum

			FROM #TABLE_RESULT
			LEFT OUTER JOIN #TEMP_F0120_WHILE ON (
				#TABLE_RESULT.employee_cd				= 	#TEMP_F0120_WHILE.employee_cd
			AND	#TABLE_RESULT.fiscal_year				= 	#TEMP_F0120_WHILE.fiscal_year
			AND	#TABLE_RESULT.treatment_applications_no	=	#TEMP_F0120_WHILE.treatment_applications_no	
			)
	
			) AS P
			Pivot(MAX(column_point_sum) FOR column_nm IN ('+@string_column+')) AS A
			ORDER BY	
			...
			BEGIN
				...
				'SELECT
					'+@str_lbl+'			
					CASE
						WHEN final_evaluation_can_edited > 0
						THEN point_sum5
						ELSE NULL
					END		AS point_sum5	
				,	CASE
			...
			BEGIN
				...
				,	rank_kinds1			
				,	CASE 
						WHEN rater_status >= 2  AND m0100_step > = 2
						THEN point_sum2	
						ELSE NULL	
					END	AS point_sum2
				,	CASE 
				...
				BEGIN
					...
					,	CASE 
						...
						END	AS rank_kinds2		
					,	CASE 
							WHEN rater_status >= 3 AND m0100_step > = 3
							THEN point_sum3	
							ELSE NULL	
						END	AS point_sum3
					,	CASE 
				...
				BEGIN
					SET @rater_str_5 = '	point_sum1
									...
									,	rank_kinds1	
									,	CASE 
											WHEN rater_status >= 2 OR (feed_back_status = 1 AND rater_status = 1)
											THEN point_sum2	
											ELSE NULL	
										END	AS point_sum2
									,	CASE 
				...
				BEGIN
					SET @rater_str_5 = '	point_sum1
									...
									,	CASE 
										...
										END	AS rank_kinds2
									,	CASE 
											WHEN rater_status >= 3 OR (feed_back_status = 1 AND rater_status = 1)
											THEN point_sum3	
											ELSE NULL	
										END	AS point_sum3
									,	CASE 
				...
				BEGIN
					SET @rater_str_5 = '	point_sum1
									...
									,	CASE 
										...
										END	AS rank_kinds3
									,	CASE 
											WHEN rater_status >= 4 OR (feed_back_status = 1 AND rater_status = 1)
											THEN point_sum4	
											ELSE NULL	
										END	AS point_sum4
									,	CASE 
				...
				END 
				-- @count_rater = 6
				IF @count_rater	= 6 AND (@authority_typ IN (3,4,5) OR @max_feedback_status = 1)
				BEGIN
					SET  @string_sql = 
					'SELECT
						'+@str_lbl+'			
						'+@rater_str_5+ '
					,	CASE 
							WHEN final_rater_status = 5 OR (feed_back_status = 1 AND rater_status = 1)
							THEN IIF(final_evaluation_can_edited > 0,point_sum5,NULL)
							ELSE NULL	
						END	AS point_sum5
					,	CASE 
							WHEN final_rater_status = 5 OR (feed_back_status = 1 AND rater_status = 1)
							THEN IIF(final_evaluation_can_edited > 0,rank_kinds5,NULL)	
							ELSE NULL	
						END	AS rank_kinds5			
					,	comment
					,	treatment_applications_no
				...
				END
				-- @count_rater = 6
				IF @count_rater	= 6 AND @authority_typ NOT IN (3,4,5)  AND @max_feedback_status <> 1
				BEGIN
					SET  @string_sql = 
					'SELECT
						'+@str_lbl+'			
		...
		BEGIN
			...
			BEGIN
				...
				'+@str_lbl+'				
					CASE 
						WHEN rater_status = 0 OR confirm_last_step = 1
						THEN IIF(final_evaluation_can_edited > 0,point_sum5,NULL)	
						ELSE NULL	
					END	AS point_sum5
				,	CASE 
			...
			BEGIN
				...
				'
				END
				IF	@count_rater	=	6 AND (@authority_cd = 3 OR @authority_cd = 4) 
				BEGIN
					SET  @string_sql = 
				'SELECT
				'+@str_lbl+'				
				...
				'
				END
				IF	@count_rater	=	6 AND (@authority_cd <> 3 AND @authority_cd <> 4) 
				BEGIN
					SET  @string_sql = 
				'SELECT
				'+@str_lbl+'				
