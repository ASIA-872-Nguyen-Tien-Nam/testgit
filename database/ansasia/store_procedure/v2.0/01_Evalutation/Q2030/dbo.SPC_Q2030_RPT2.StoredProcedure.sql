...
--****************************************************************************************
CREATE PROCEDURE [SPC_Q2030_RPT2]
...
BEGIN
	...
	BEGIN
		...
		LEFT JOIN S0022 ON(
			S0022.company_cd		=	@P_company_cd
		AND @authority_cd			=	authority_cd 
		AND S0022.organization_cd_1	=	M0020.organization_cd_1
		AND S0022.organization_cd_2	=	M0020.organization_cd_2
		AND S0022.organization_cd_3	=	M0020.organization_cd_3
		AND S0022.organization_cd_4	=	M0020.organization_cd_4
		AND S0022.organization_cd_5	=	M0020.organization_cd_5
		)
		WHERE 
	...
	BEGIN
		...
		WHERE 
			F0200.treatment_applications_no	=	@P_treatment_applications_no
		AND #F0030.type_rater = @P_evaluation_step
	END
	-- add by viettd 2022/08/16
	UPDATE #DATA SET 
		sheet_cd = ISNULL(F0032_SHEET_MAX.sheet_cd,0)
	FROM #DATA
	INNER JOIN (
		SELECT 
	...
	FROM #DATA			
	WHERE 
		rater_employee_cd	=	@P_unit_display
	AND rank_cd				=	@P_rank_cd
	--
	INSERT INTO #RESULT
	SELECT
