DROP PROCEDURE [SPC_S0030_LST1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [SPC_S0030_LST1]
/****************************************************************************************************
 *
 *  処理概要:  Search
 *
...
BEGIN
	...
	--
	SELECT 
		@P_authority_typ	=	ISNULL(S0010.authority_typ,0) 
	,	@P_authority_cd		=	ISNULL(S0010.authority_cd,0)
	...
	,	organization_ab_nm_1	nvarchar(10)
	,	organization_nm_2		nvarchar(50)
	,	organization_ab_nm_2	nvarchar(10)
	,	organization_nm_3		nvarchar(50)
	,	organization_ab_nm_3	nvarchar(10)
	,	organization_nm_4		nvarchar(50)
	,	organization_ab_nm_4	nvarchar(10)
	,	organization_nm_5		nvarchar(50)
	,	organization_ab_nm_5	nvarchar(10)
	,	job_cd					smallint
	,	job_nm					nvarchar(50)
	,	job_ab_nm				nvarchar(10)
	,	position_cd				INT
	...
	,	M0070.belong_cd4
	,	M0070.belong_cd5
	,	M0020_CD1.organization_nm		AS organization_nm_1
	,	M0020_CD1.organization_ab_nm	AS organization_ab_nm_1
	,	M0020_CD2.organization_nm		AS organization_nm_2
	,	M0020_CD2.organization_ab_nm	AS organization_ab_nm_2
	,	M0020_CD3.organization_nm		AS organization_nm_3
	,	M0020_CD3.organization_ab_nm	AS organization_ab_nm_3
	,	M0020_CD4.organization_nm		AS organization_nm_4
	,	M0020_CD4.organization_ab_nm	AS organization_ab_nm_4
	,	M0020_CD5.organization_nm		AS organization_nm_5
	,	M0020_CD5.organization_ab_nm	AS organization_ab_nm_5
	,	M0070.job_cd
	,	M0030.job_nm
	,	M0030.job_ab_nm
	,	M0070.position_cd
	,	M0040.position_nm
	,	M0040.position_ab_nm
	,	M0070.grade
	,	M0050.grade_nm
	,	M0070.office_cd
	,	M0010.office_nm
	...
	AND	S0010.authority_cd		=	S0020.authority_cd
	)
	LEFT JOIN S0020 AS S0020_LOGIN ON (
		M0070.company_cd		=	S0020_LOGIN.company_cd
	AND	@P_authority_cd			=	S0020_LOGIN.authority_cd
	)
	...
	AND	M0070.del_datetime IS NULL	
	AND ISNULL(S0010.authority_typ,0) <> 5				-- add by viettd 2021/10/12
	AND (
		@P_employee_cd = ''
	...
	OR	(@P_authority_cd_screen <> 0 AND S0010.authority_cd = @P_authority_cd_screen)
	)
	AND (
			@P_authority_typ	<> 3
		OR (
				@P_authority_typ	= 3 
				AND(
					(ISNULL(S0020_LOGIN.use_typ,0)	<> 1 OR ( ISNULL(S0020_LOGIN.use_typ,0)  = 1 AND ISNULL(M0040.arrange_order,0) > @arrange_order ))
				)
			)
	)
	AND (
	...
	,	belong_cd5	
	,	CASE
			WHEN organization_ab_nm_1 = '' OR organization_ab_nm_1 IS NULL
			THEN organization_nm_1
			ELSE organization_ab_nm_1
		END	AS organization_nm_1	
	,	CASE
			WHEN organization_ab_nm_2 = '' OR organization_ab_nm_2 IS NULL
			THEN organization_nm_2
			ELSE organization_ab_nm_2
		END	AS organization_nm_2	
	,	CASE
			WHEN organization_ab_nm_3 = '' OR organization_ab_nm_3 IS NULL
			THEN organization_nm_3
			ELSE organization_ab_nm_3
		END	AS organization_nm_3	
	,	CASE
			WHEN organization_ab_nm_4 = '' OR organization_ab_nm_4 IS NULL
			THEN organization_nm_4
			ELSE organization_ab_nm_4
		END	AS organization_nm_4	
	,	CASE
			WHEN organization_ab_nm_5 = '' OR organization_ab_nm_5 IS NULL
			THEN organization_nm_5
			ELSE organization_ab_nm_5
		END	AS organization_nm_5	
	,	job_cd				
	,	CASE
			WHEN job_ab_nm = '' OR job_ab_nm IS NULL
			THEN job_nm
			ELSE job_ab_nm
		END	AS job_nm	
	...
	,	position_cd				
	,	CASE
			WHEN position_ab_nm = '' OR position_ab_nm IS NULL
			THEN position_nm
			ELSE position_ab_nm
		END	AS position_nm		
	,	grade				
	...
	,	office_cd			
	,	CASE
			WHEN office_ab_nm = '' OR office_ab_nm IS NULL
			THEN office_nm
			ELSE office_ab_nm
		END	AS office_nm				
	,	user_id				
