DROP PROCEDURE [SPC_1on1_GET_SEARCH_CONDITIONS_INQ1]
GO
SET ANSI_NULLS ON
GO
...
--* 
--****************************************************************************************
CREATE PROCEDURE [SPC_1on1_GET_SEARCH_CONDITIONS_INQ1]
...
BEGIN
	...
	--
	CREATE TABLE #LIST_GROUP_1ON1_ANALYSIS (
		id								int			identity(1,1)
	...
	INSERT INTO #LIST_GROUP_1ON1_ANALYSIS
	...
	SELECT 
		CAST(@w_fiscal_year AS nvarchar(4)) + IIF(@w_language = 2,' Year','年度')		AS	fiscal_year_nm
	,	STUFF((SELECT N', '+ CAST(ISNULL(M2600.[1on1_group_nm],'') AS NVARCHAR(20))
								FROM #LIST_GROUP_1ON1_ANALYSIS
								LEFT OUTER JOIN M2600 ON (
									@P_company_cd					=	M2600.company_cd
								AND #LIST_GROUP_1ON1_ANALYSIS.group_cd_1on1	=	M2600.[1on1_group_cd]
	...
	,	group_cd_1on1_nm						AS	group_cd_1on1_nm	
	...
	,	CASE 
			WHEN @w_combination_vertical = 1 THEN IIF(@w_language = 2,'Implementation Status',N'実施状況')
			WHEN @w_combination_vertical = 2 THEN IIF(@w_language = 2,'Adequacy',N'充実度')
			WHEN @w_combination_vertical = 3 THEN IIF(@w_language = 2,'Questionnaire Results',N'アンケート結果')
			ELSE N''
		END										AS	 combination_vertical	--組み合わせ 1
	,	CASE 
			WHEN @w_combination_horizontal = 1 THEN IIF(@w_language = 2,'Implementation Status',N'実施状況')
			WHEN @w_combination_horizontal = 2 THEN IIF(@w_language = 2,'Adequacy',N'充実度')
			WHEN @w_combination_horizontal = 3 THEN IIF(@w_language = 2,'Questionnaire Results',N'アンケート結果')
			ELSE N''
	...
	DROP TABLE #LIST_GROUP_1ON1_ANALYSIS
