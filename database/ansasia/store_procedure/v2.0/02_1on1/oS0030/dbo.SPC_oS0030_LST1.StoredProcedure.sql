DROP PROCEDURE [SPC_oS0030_LST1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [SPC_oS0030_LST1]
/****************************************************************************************************
 *
 *  処理概要:  Search
 *
...
BEGIN
	...
	--
	SELECT 
		@P_authority_typ	=	ISNULL(S0010.[1on1_authority_typ],0) 
	,	@P_authority_cd		=	ISNULL(S0010.[1on1_authority_cd],0)
	...
	SET @w_1on1_organization_cnt = [dbo].FNC_GET_NUMBER_OF_ORGANIZATION(@P_company_cd,@P_authority_cd,2)
	-- GET @w_organization_belong_person_typ add by viettd 2021/06/03
	SET @w_organization_belong_person_typ = [dbo].FNC_GET_ORGANIZATION_BELONG_PERSON_TYP(@P_company_cd,@P_authority_cd,2)
	--
	CREATE TABLE #M0070 (
		id						int	identity(1, 1)
	,	employee_cd				nvarchar(10)
	...
	,	M0070.belong_cd5
	,	CASE 
			WHEN ISNULL(M0020_CD1.organization_ab_nm,'') = ''
			THEN ISNULL(M0020_CD1.organization_nm,'')
			ELSE ISNULL(M0020_CD1.organization_ab_nm,'')
		END		AS organization_nm_1
	,	CASE 
			WHEN ISNULL(M0020_CD2.organization_ab_nm,'') = ''
			THEN ISNULL(M0020_CD2.organization_nm,'')
			ELSE ISNULL(M0020_CD2.organization_ab_nm,'')
		END		AS organization_nm_2

	,	CASE 
			WHEN ISNULL(M0020_CD3.organization_ab_nm,'') = ''
			THEN ISNULL(M0020_CD3.organization_nm,'')
			ELSE ISNULL(M0020_CD3.organization_ab_nm,'')
		END		AS organization_nm_3
	,	CASE 
			WHEN ISNULL(M0020_CD4.organization_ab_nm,'') = ''
			THEN ISNULL(M0020_CD4.organization_nm,'')
			ELSE ISNULL(M0020_CD4.organization_ab_nm,'')
		END		AS organization_nm_4
	,	CASE 
			WHEN ISNULL(M0020_CD5.organization_ab_nm,'') = ''
			THEN ISNULL(M0020_CD5.organization_nm,'')
			ELSE ISNULL(M0020_CD5.organization_ab_nm,'')
	...
	,	CASE 
		...
		END		AS office_nm
	,	S0010.user_id
	,	ISNULL(S0010.[1on1_authority_cd], '')
	,	S2020.authority_nm
	FROM M0070
	LEFT JOIN M0060 ON (
	...
	LEFT JOIN S2020 ON (
		M0070.company_cd	=	S2020.company_cd
	AND	S0010.[1on1_authority_cd]		=	S2020.authority_cd
	)
	LEFT JOIN S2020 AS S2020_LOGIN ON (
		M0070.company_cd		=	S2020_LOGIN.company_cd
	AND	@P_authority_cd			=	S2020_LOGIN.authority_cd
	)
	WHERE
		M0070.company_cd	=	@P_company_cd
	AND	M0070.del_datetime IS NULL
	AND ISNULL(S0010.[1on1_authority_typ],0) <> 5		-- add by viettd 2021/10/12
	AND (
		@P_employee_cd = ''
	...
	OR	(
			@P_authority_cd_screen <> 0 AND S0010.[1on1_authority_cd] = @P_authority_cd_screen
		AND	S0010.[1on1_authority_typ] = 3		--add vietdt is 2022/01/24
		)
	)
	AND (
			@P_authority_typ	<> 3
		OR (
				@P_authority_typ	= 3 
				AND(
					(ISNULL(S2020_LOGIN.use_typ,0)	<> 1 OR ( ISNULL(S2020_LOGIN.use_typ,0)  = 1 AND ISNULL(M0040.arrange_order,0) > @arrange_order ))
				)
			)
	)
	...
	BEGIN
		...
		WHERE
			ISNULL(S0010.[1on1_authority_cd],0) <> 0
		AND S0010.del_datetime IS NULL
	END
	-- get permission
	INSERT INTO #TABLE_ORGANIZATION
	EXEC [dbo].SPC_REFER_ORGANIZATION_FND1 @P_json,@P_user_id,@P_company_cd,2
	--
	...
	BEGIN
		...
		END
		ELSE IF NOT (@P_authority_typ = 3 AND @w_1on1_organization_cnt = 0 AND @w_organization_belong_person_typ = 0) -- edited by viettd 2021/06/03
		BEGIN
			DELETE D FROM #M0070 AS D
			FULL OUTER JOIN #TABLE_ORGANIZATION AS S ON (
				D.belong_cd1			=	S.organization_cd_1
