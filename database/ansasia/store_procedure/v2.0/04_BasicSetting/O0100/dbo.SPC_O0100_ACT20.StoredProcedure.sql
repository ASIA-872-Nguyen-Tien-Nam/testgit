DROP PROCEDURE [SPC_O0100_ACT20]
GO
SET ANSI_NULLS ON
GO
...
--****************************************************************************************
--*   											
--*  処理概要/process overview	:	O0100 社員マスタ (給与)
--*  
...
CREATE PROCEDURE [SPC_O0100_ACT20]
...
BEGIN
	...
	BEGIN TRY
		...
		,	REPLACE(RTRIM(LTRIM(base_salary)),'.00','')
		,	REPLACE(RTRIM(LTRIM(basic_annual_income)),'.00','')										
		...
		END
		-------------------------------------------------
		--VALIDATE MAXLENGHT base_salary
		INSERT INTO @ERR_TBL
		SELECT
			28				
		,	'base_salary'				
		...
		,	0				
		,	0				
		,	IIF(@P_language = N'en',N'Base Balary',N'基本給')	
		FROM #TEMP 
		WHERE
			(LEN(#TEMP.base_salary)>10)
		...
		WHERE
			(#TEMP.base_salary <> '')
		AND (#TEMP.base_salary LIKE '%[^0-9]%')
		-------------------------------------------------
		--VALIDATE MAXLENGHT basic_annual_income
		INSERT INTO @ERR_TBL
		SELECT
			28				
		,	'basic_annual_income'				
		...
		,	0				
		,	0				
		,	IIF(@P_language = N'en',N'Basic Annual Income',N'基本年収')	
		FROM #TEMP 
		WHERE
			(LEN(#TEMP.basic_annual_income)>10)
		--VALIDATE NUMERIC basic_annual_income < 0
		INSERT INTO @ERR_TBL
		SELECT
			13				
		,	'basic_annual_income'				
		...
		FROM #TEMP 
		WHERE
			(#TEMP.basic_annual_income <> '')
		AND (#TEMP.basic_annual_income LIKE '%[^0-9]%')
		...
		BEGIN
			DELETE W FROM  WRK_M0092 AS W LEFT JOIN  #TEMP AS T ON W.company_cd = @P_company_cd AND W.exec_user = T.exec_user  WHERE T.company_cd IS NOT NULL
			--
			INSERT INTO WRK_M0092 
			SELECT 
				#TEMP.exec_user
			,	#TEMP.company_cd
			,	#TEMP.employee_cd						
		...
		ELSE
		BEGIN
			DELETE W FROM  WRK_M0092 AS W LEFT JOIN  #TEMP AS T ON W.company_cd = @P_company_cd AND W.exec_user = T.exec_user  WHERE W.exec_user LIKE '%WRK_M0092%'
			
			--
			INSERT INTO WRK_M0092 
			SELECT 
				CONCAT('WRK_M0092', CAST(@P_no AS NVARCHAR(10)))
			,	0
			,	N''								
			,	0	
			,	0					
		...
		BEGIN
			--
			IF (EXISTS(SELECT 1 FROM @ERR_TBL) OR EXISTS(SELECT 1 FROM WRK_M0092 WHERE exec_user LIKE '%WRK_M0092%'))
			BEGIN
				TRUNCATE TABLE WRK_M0092
				--
				GOTO COMPLETE_QUERY
			END
			...
			UPDATE M0092
			SET 
				base_salary					=	WRK_M0092.base_salary
			,	basic_annual_income			=	WRK_M0092.basic_annual_income
			,	upd_user					=	@P_cre_user		
			,	upd_ip						=	@P_cre_ip	
			,	upd_prg						=	N'O0100'	
			,	upd_datetime				=	@w_time	
			...
			,	del_datetime				=	NULL
			FROM M0092 
			INNER JOIN WRK_M0092 ON (
				M0092.company_cd	=	WRK_M0092.company_cd
			AND M0092.employee_cd	=	WRK_M0092.employee_cd
			AND WRK_M0092.company_cd    = @P_company_cd
			)		

			-- INSERT M0092
			INSERT INTO M0092
			SELECT  
				WRK_M0092.company_cd
			,	WRK_M0092.employee_cd
			,	WRK_M0092.base_salary					
			,	WRK_M0092.basic_annual_income				
			,	@P_cre_user			
			,	@P_cre_ip	
			,	N'O0100'		
			,	@w_time
			...
			WHERE 
				M0092.employee_cd IS NULL
			AND	WRK_M0092.employee_cd <> N''
			AND WRK_M0092.company_cd    = @P_company_cd
