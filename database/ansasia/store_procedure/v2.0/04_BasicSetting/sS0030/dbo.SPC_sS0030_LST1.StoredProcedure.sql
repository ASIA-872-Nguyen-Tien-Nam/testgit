DROP PROCEDURE [SPC_sS0030_LST1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [SPC_sS0030_LST1]
/****************************************************************************************************
 *
 *  処理概要:  Search
 *
...
BEGIN
	...
	SELECT 
		@P_authority_typ	=	ISNULL(S0010.setting_authority_typ,0) 
	,	@P_authority_cd		=	ISNULL(S0010.setting_authority_cd,0)
	...
	SET @w_setting_organization_cnt = [dbo].FNC_GET_NUMBER_OF_ORGANIZATION(@P_company_cd,@P_authority_cd,4)
	-- GET @w_organization_belong_person_typ add by viettd 2021/06/03
	SET @w_organization_belong_person_typ = [dbo].FNC_GET_ORGANIZATION_BELONG_PERSON_TYP(@P_company_cd,@P_authority_cd,4)
	--
	CREATE TABLE #M0070 (
		id						int	identity(1, 1)
	,	employee_cd				nvarchar(10)
	...
	,	S0010.user_id
	,	ISNULL(S0010.setting_authority_cd, '')
	,	S9020.authority_nm
	FROM M0070
	LEFT JOIN M0060 ON (
	...
	LEFT JOIN S9020 ON (
		M0070.company_cd				=	S9020.company_cd
	AND	S0010.setting_authority_cd		=	S9020.authority_cd
	)
	LEFT JOIN S9020 AS S9020_LOGIN ON (
		M0070.company_cd		=	S9020_LOGIN.company_cd
	AND	@P_authority_cd			=	S9020_LOGIN.authority_cd
	)
	...
	AND	M0070.del_datetime IS NULL
	AND ISNULL(S0010.setting_authority_typ,0) <> 5				-- add by viettd 2021/10/12
	AND (
		@P_employee_cd = ''
	...
	OR	(@P_authority_cd_screen <> 0 AND S0010.setting_authority_cd = @P_authority_cd_screen)
	)
	AND (
			@P_authority_typ	<> 3
		OR (
				@P_authority_typ	= 3 
				AND(
					(ISNULL(S9020_LOGIN.use_typ,0)	<> 1 OR ( ISNULL(S9020_LOGIN.use_typ,0)  = 1 AND ISNULL(M0040.arrange_order,0) > @arrange_order ))
				)
			)
	)
	AND (
	...
	BEGIN
		...
		WHERE
			ISNULL(S0010.setting_authority_cd,0) <> 0
		AND S0010.del_datetime IS NULL
	END
	--
	-- get permission
	INSERT INTO #TABLE_ORGANIZATION
	EXEC [dbo].SPC_REFER_ORGANIZATION_FND1 @P_json,@P_user_id,@P_company_cd,4
