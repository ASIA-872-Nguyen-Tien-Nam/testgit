IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SPC_rQ3030_FND1]') AND type IN (N'P', N'PC'))
BEGIN
    DROP PROCEDURE [dbo].[SPC_rQ3030_FND1]
END
GO
SET ANSI_NULLS ON
GO
...
--****************************************************************************************
--*   											
--*  処理概要/process overview	:	rQ3030_分析（リアクション）
--*  
...
--*   					
--****************************************************************************************
CREATE PROCEDURE [SPC_rQ3030_FND1]
...
BEGIN
	...
	--
	,	reaction_total					money
	,	reaction_point_total			money
	,	point							money
	)
	-- get reactions of approvers
	CREATE TABLE #F4204_TABLE_REACTIONS (
		id								int		identity(1,1)
	,	company_cd						smallint
	,	fiscal_year						smallint
	,	employee_cd						nvarchar(10)
	...
	,	reaction_cd						smallint
	,	point							money
	,	employee_role					smallint		-- 1.承認者 2.閲覧者
	)
	-- #F4100_TABLE
	...
	SET @w_employee_role	=	ISNULL(JSON_VALUE(@P_json,'$.employee_role'),0)			-- 対象
	...
	SET @w_viewer_cd		=	ISNULL(JSON_VALUE(@P_json,'$.viewer_cd'),'')			-- 閲覧者
	...
	END
	-- INSERT DATA INTO #LIST_VIEWER
	IF @w_viewer_cd IS NOT NULL AND @w_viewer_cd <> ''
	BEGIN
		INSERT INTO #LIST_VIEWER
		SELECT 
			F4120.company_cd
		,	F4120.fiscal_year
		,	F4120.employee_cd
		,	F4120.report_kind
		,	F4120.report_no
		FROM F4120 WITH(NOLOCK)
		WHERE
			F4120.company_cd	=	@P_company_cd
		AND F4120.fiscal_year	=	@w_fiscal_year
		AND (
			(@w_report_kind <= 0)
		OR	(@w_report_kind > 0 AND F4120.report_kind = @w_report_kind)
		)
		AND F4120.viewer_employee_cd	=	@w_viewer_cd
	...
	BEGIN
		DELETE D FROM #F4200_TABLE AS D
		LEFT OUTER JOIN #LIST_VIEWER AS S ON (
			D.company_cd		=	@P_company_cd
		AND D.employee_cd		=	S.employee_cd
		)
		WHERE
	...
	BEGIN
		...
		WHERE
			S.group_cd IS NULL
	END
	-- INSERT APPROVER REACTIONS
	IF @w_employee_role = 1 -- 承認者
	BEGIN
		--1
		INSERT INTO #F4204_TABLE_REACTIONS
		SELECT 
			#F4200_TABLE.company_cd		
		,	#F4200_TABLE.fiscal_year			
		,	#F4200_TABLE.employee_cd			
		,	#F4200_TABLE.report_kind			
		,	#F4200_TABLE.report_no			
		,	ISNULL(F4200.approver_employee_cd_1,'')
		,	0	
		,	0	
		,	1
		FROM #F4200_TABLE
		LEFT OUTER JOIN F4200 ON (
			#F4200_TABLE.company_cd			= F4200.company_cd	
		AND	#F4200_TABLE.fiscal_year		= F4200.fiscal_year	
		AND	#F4200_TABLE.employee_cd		= F4200.employee_cd	
		AND	#F4200_TABLE.report_kind		= F4200.report_kind	
		AND	#F4200_TABLE.report_no			= F4200.report_no	
		)
		WHERE 
			F4200.company_cd	=	@P_company_cd
		AND F4200.fiscal_year	=	@w_fiscal_year
		AND F4200.del_datetime IS NULL
		AND F4200.approver_employee_cd_1 <>	''
		-- 2
		INSERT INTO #F4204_TABLE_REACTIONS
		SELECT 
			#F4200_TABLE.company_cd		
		...
		,	#F4200_TABLE.report_kind			
		,	#F4200_TABLE.report_no			
		,	ISNULL(F4200.approver_employee_cd_2,'')
		,	0	
		,	0	
		,	1
		FROM #F4200_TABLE
		...
		AND F4200.fiscal_year	=	@w_fiscal_year
		AND F4200.del_datetime IS NULL
		AND F4200.approver_employee_cd_2 <>	''
		-- 3
		INSERT INTO #F4204_TABLE_REACTIONS
		SELECT 
			#F4200_TABLE.company_cd		
		...
		,	#F4200_TABLE.report_kind			
		,	#F4200_TABLE.report_no			
		,	ISNULL(F4200.approver_employee_cd_3,'')
		,	0	
		,	0	
		,	1
		FROM #F4200_TABLE
		...
		AND F4200.fiscal_year	=	@w_fiscal_year
		AND F4200.del_datetime IS NULL
		AND F4200.approver_employee_cd_3 <>	''
		-- 4
		INSERT INTO #F4204_TABLE_REACTIONS
		SELECT 
			#F4200_TABLE.company_cd		
		...
		,	#F4200_TABLE.report_kind			
		,	#F4200_TABLE.report_no			
		,	ISNULL(F4200.approver_employee_cd_4,'')
		,	0	
		,	0	
		,	1
		FROM #F4200_TABLE
		...
		AND F4200.fiscal_year	=	@w_fiscal_year
		AND F4200.del_datetime IS NULL
		AND F4200.approver_employee_cd_4 <>	''
	END
	IF @w_employee_role = 2 -- 閲覧者
	BEGIN
		-- INSERT VIEWER REACTIONS
		INSERT INTO #F4204_TABLE_REACTIONS
		SELECT 
			#F4200_TABLE.company_cd		
		,	#F4200_TABLE.fiscal_year			
		...
		,	#F4200_TABLE.report_kind			
		,	#F4200_TABLE.report_no			
		,	ISNULL(F4120.viewer_employee_cd,'')
		,	0	
		,	0
		,	2
		FROM #F4200_TABLE
		LEFT OUTER JOIN F4120 ON (
			#F4200_TABLE.company_cd			= F4120.company_cd	
		AND	#F4200_TABLE.fiscal_year		= F4120.fiscal_year	
		AND	#F4200_TABLE.employee_cd		= F4120.employee_cd	
		AND	#F4200_TABLE.report_kind		= F4120.report_kind	
		AND	#F4200_TABLE.report_no			= F4120.report_no
		)
		WHERE 
			F4120.company_cd	=	@P_company_cd
		AND F4120.fiscal_year	=	@w_fiscal_year
		AND F4120.del_datetime IS NULL
	END
	-- UPDATE POINT FOR #F4204_TABLE_REACTIONS
	UPDATE #F4204_TABLE_REACTIONS SET 
		point = ISNULL(F4204_RECTION.point,0)
	FROM #F4204_TABLE_REACTIONS
	INNER JOIN (
		SELECT 
		...
		,	F4204.reaction_no		AS	reaction_no
		,	ISNULL(M4123.point,0)	AS	point
		FROM F4204
		LEFT OUTER JOIN M4123 ON (
			F4204.company_cd		=	M4123.company_cd
		AND 2						=	M4123.mark_kbn
		AND F4204.reaction_cd		=	M4123.mark_cd
		AND M4123.del_datetime IS NULL
		)
		WHERE 
			F4204.company_cd		=	@P_company_cd
		AND F4204.fiscal_year		=	@w_fiscal_year
		AND F4204.del_datetime IS NULL
	) AS F4204_RECTION ON (
		#F4204_TABLE_REACTIONS.company_cd	=	F4204_RECTION.company_cd
	AND #F4204_TABLE_REACTIONS.fiscal_year	=	F4204_RECTION.fiscal_year
	AND #F4204_TABLE_REACTIONS.employee_cd	=	F4204_RECTION.employee_cd
	AND #F4204_TABLE_REACTIONS.report_kind	=	F4204_RECTION.report_kind
	AND #F4204_TABLE_REACTIONS.report_no	=	F4204_RECTION.report_no
	AND #F4204_TABLE_REACTIONS.reaction_no	=	F4204_RECTION.reaction_no
	)
	-- UPDATE REACTIONS SCORE INTO  #F4200_TABLE
	UPDATE #F4200_TABLE SET 
		reaction_total			=	ISNULL(F4204_REACTION_TOTAL.reaction_total,0)
	,	reaction_point_total	=	ISNULL(F4204_REACTION_TOTAL.reaction_point_total,0)
	FROM #F4200_TABLE
	INNER JOIN (
		SELECT 
			#F4204_TABLE_REACTIONS.company_cd		
		,	#F4204_TABLE_REACTIONS.fiscal_year			
		,	#F4204_TABLE_REACTIONS.employee_cd			
		,	#F4204_TABLE_REACTIONS.report_kind			
		,	#F4204_TABLE_REACTIONS.report_no	
		,	COUNT(#F4204_TABLE_REACTIONS.reaction_no)	AS	reaction_total
		,	SUM(#F4204_TABLE_REACTIONS.point)			AS	reaction_point_total
		FROM #F4204_TABLE_REACTIONS
		GROUP BY
			#F4204_TABLE_REACTIONS.company_cd		
		,	#F4204_TABLE_REACTIONS.fiscal_year			
		...
		,	#F4204_TABLE_REACTIONS.report_kind			
		,	#F4204_TABLE_REACTIONS.report_no	
	) AS F4204_REACTION_TOTAL ON (
		#F4200_TABLE.company_cd		=	F4204_REACTION_TOTAL.company_cd	
	AND	#F4200_TABLE.fiscal_year	=	F4204_REACTION_TOTAL.fiscal_year	
	AND	#F4200_TABLE.employee_cd	=	F4204_REACTION_TOTAL.employee_cd	
	AND	#F4200_TABLE.report_kind	=	F4204_REACTION_TOTAL.report_kind	
	AND	#F4200_TABLE.report_no		=	F4204_REACTION_TOTAL.report_no	
	)
	-- GET AVG
	UPDATE #F4200_TABLE SET 
		point	=	CASE
						WHEN reaction_total <> 0
						THEN ROUND(reaction_point_total / reaction_total,2)
						ELSE 0
					END
	FROM #F4200_TABLE
...
COMPLETED:	
	...
	BEGIN
		...
		--[1]
		SELECT
			IIF(@w_language = 2, 'Reactions Score', 'リアクション点数')		AS	sufficiency_nm
	...
	BEGIN
		...
		-- GET @w_mark_type
		SET @w_mark_type = 2
		SET @w_m4123_cnt = (SELECT COUNT(1) FROM M4123 WHERE company_cd = @P_company_cd AND mark_kbn = @w_mark_type AND del_datetime IS NULL)	
		-- SET
		SET @w_i = @w_m4123_cnt
		WHILE @w_i >= 1
		BEGIN
			...
			--
			SELECT
				@w_mark_cd		= ISNULL(M4123.mark_cd,0)
			,	@w_point_from	= ISNULL(M4123.point,0)
			FROM M4123
			WHERE 
				M4123.company_cd	=	@P_company_cd
			AND M4123.mark_kbn		=	@w_mark_type
			AND M4123.item_no		=	@w_i
			--
			SET @w_i_minus = @w_i - 1
			--
			SELECT 
				@w_point_to = ISNULL(M4123.point,0) 
			FROM M4123 
			WHERE 
				M4123.company_cd	=	@P_company_cd
			AND M4123.mark_kbn		=	@w_mark_type
			AND M4123.item_no		=	@w_i_minus
		...
		SELECT 
			TOP 1
			ISNULL(M4123.item_no,0)		AS	item_no
		,	ISNULL(M4123.mark_cd,0)		AS	mark_Rcd
		,	ISNULL(M4123.point,0)		AS	point
		FROM M4123
		WHERE 
			M4123.company_cd	=	@P_company_cd
	...
	DROP TABLE #LIST_VIEWER
	DROP TABLE #LIST_GROUP
	DROP TABLE #F4100_TABLE
	DROP TABLE #F4204_TABLE_REACTIONS
	DROP TABLE #TOTAL
