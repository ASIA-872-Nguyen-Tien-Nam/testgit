DROP PROCEDURE [SPC_rS0030_LST1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [SPC_rS0030_LST1]
/****************************************************************************************************
 *
 *  処理概要:  Search
 *
...
BEGIN
	...
	,	@w_weeklyreport_organization_cnt	INT					=	0	-- 0.view all 1.only view organization choiced
	...
	SET @w_weeklyreport_organization_cnt = [dbo].FNC_GET_NUMBER_OF_ORGANIZATION(@P_company_cd,@w_report_authority_cd,5)
	...
	,	CASE 
		...
		END		AS office_nm
	,	S0010.user_id
	,	ISNULL(S0010.[report_authority_cd], '')
	,	S4020.authority_nm
	FROM M0070
	LEFT JOIN M0060 ON (
	...
	LEFT JOIN S4020 ON (
		M0070.company_cd	=	S4020.company_cd
	AND	S0010.[report_authority_cd]		=	S4020.authority_cd
	)
	LEFT JOIN S4020 AS S4020_LOGIN ON (
		M0070.company_cd		=	S4020_LOGIN.company_cd
	AND	@w_report_authority_cd	=	S4020_LOGIN.authority_cd
	)
	WHERE
		M0070.company_cd	=	@P_company_cd
	AND	M0070.del_datetime IS NULL
	AND ISNULL(S0010.[report_authority_typ],0) <> 5		-- add by viettd 2021/10/12
	AND (
		@P_employee_cd = ''
	...
	OR	(
			@P_authority_cd_screen <> 0 AND S0010.[report_authority_cd] = @P_authority_cd_screen
		AND	S0010.[report_authority_typ] = 3		--add vietdt is 2022/01/24
		)
	)
	AND (
			@w_report_authority_typ	<> 3
		OR (
				@w_report_authority_typ	= 3 
				AND(
					(ISNULL(S4020_LOGIN.use_typ,0)	<> 1 OR ( ISNULL(S4020_LOGIN.use_typ,0)  = 1 AND ISNULL(M0040.arrange_order,0) > @arrange_order ))
				)
			)
	)
	...
	BEGIN
		...
		WHERE
			ISNULL(S0010.[report_authority_cd],0) <> 0
		AND S0010.del_datetime IS NULL
	END
	-- get permission
	INSERT INTO #TABLE_ORGANIZATION
	EXEC [dbo].SPC_REFER_ORGANIZATION_FND1 @P_json,@P_user_id,@P_company_cd,5
	--
	...
	BEGIN
		...
		END
		ELSE IF NOT (@w_report_authority_typ = 3 AND @w_weeklyreport_organization_cnt = 0 AND @w_organization_belong_person_typ = 0) -- edited by viettd 2021/06/03
		BEGIN
			DELETE D FROM #M0070 AS D
			FULL OUTER JOIN #TABLE_ORGANIZATION AS S ON (
				D.belong_cd1			=	S.organization_cd_1
