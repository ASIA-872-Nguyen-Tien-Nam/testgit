DROP PROCEDURE [dbo].[SPC_eS0030_LST1]
GO
SET ANSI_NULLS ON
GO
...
--*  
--*  作成日/create date			:	2024/03/14
--*　作成者/creater				:	manhnd						
--*  作成内容/create content		:	copy content store from ss0030 => es0030	 					
--*	 更新日/update date			:  	
--*	 更新者/updater				:　 
--*	 更新内容/update content		:	
--****************************************************************************************
CREATE PROCEDURE [dbo].[SPC_eS0030_LST1]
 	@P_company_cd							SMALLINT			=	0
, 	@P_employee_cd							NVARCHAR(10)		=	N''
,	@P_employee_nm							NVARCHAR(101)		=	N''
,	@P_organization_cd_1					NVARCHAR(20)		=	N''
,	@P_organization_cd_2					NVARCHAR(20)		=	N''
,	@P_organization_cd_3					NVARCHAR(20)		=	N''
,	@P_organization_cd_4					NVARCHAR(20)		=	N''
,	@P_organization_cd_5					NVARCHAR(20)		=	N''
,	@P_position_cd							INT					=	0	
,	@P_check_authority						SMALLINT			=	0
,	@P_authority_cd_screen					SMALLINT			=	0	
,	@P_employee_typ							SMALLINT			=	0	
,	@P_user_id								NVARCHAR(50)		=	N''
,	@P_page_size							INT					=	50
,	@P_page									INT					=	1
...
BEGIN
	...
	,	@w_setting_organization_cnt			INT					=	0	-- 0.view all 1.only view organization choiced
	,	@w_organization_belong_person_typ	SMALLINT			=	0
	--
	SET @w_time = CONVERT(date, @w_time)
	SELECT 
		@P_authority_typ	=	ISNULL(S0010.empinfo_authority_typ,0) 
	,	@P_authority_cd		=	ISNULL(S0010.empinfo_authority_cd,0)
	...
	SET @w_setting_organization_cnt = [dbo].FNC_GET_NUMBER_OF_ORGANIZATION(@P_company_cd,@P_authority_cd,6)
	-- GET @w_organization_belong_person_typ add by viettd 2021/06/03
	SET @w_organization_belong_person_typ = [dbo].FNC_GET_ORGANIZATION_BELONG_PERSON_TYP(@P_company_cd,@P_authority_cd,6)
	--
	CREATE TABLE #M0070 (
		id						int	identity(1, 1)
	...
	,	M0070.belong_cd4
	,	M0070.belong_cd5
	,	ISNULL(IIF(CAST(M0020_CD1.organization_ab_nm as nvarchar(50))= '',NUll,CAST(M0020_CD1.organization_ab_nm as nvarchar(50))),M0020_CD1.organization_nm)	AS organization_nm_1
	,	ISNULL(IIF(CAST(M0020_CD2.organization_ab_nm as nvarchar(50))= '',NUll,CAST(M0020_CD2.organization_ab_nm as nvarchar(50))),M0020_CD2.organization_nm)	AS organization_nm_2
	,	ISNULL(IIF(CAST(M0020_CD3.organization_ab_nm as nvarchar(50))= '',NUll,CAST(M0020_CD3.organization_ab_nm as nvarchar(50))),M0020_CD3.organization_nm)	AS organization_nm_3
	,	ISNULL(IIF(CAST(M0020_CD4.organization_ab_nm as nvarchar(50))= '',NUll,CAST(M0020_CD4.organization_ab_nm as nvarchar(50))),M0020_CD4.organization_nm)	AS organization_nm_4
	,	ISNULL(IIF(CAST(M0020_CD5.organization_ab_nm as nvarchar(50))= '',NUll,CAST(M0020_CD5.organization_ab_nm as nvarchar(50))),M0020_CD5.organization_nm)	AS organization_nm_5
	,	M0070.job_cd
	,	ISNULL(IIF(CAST(M0030.job_ab_nm as nvarchar(50)) = '',NUll,CAST(M0030.job_ab_nm as nvarchar(50))),M0030.job_nm)											AS job_nm
	,	M0070.position_cd
	,	ISNULL(IIF(CAST(M0040.position_ab_nm as nvarchar(50)) = '',NUll,CAST(M0040.position_ab_nm as nvarchar(50))),M0040.position_nm)							AS position_nm
	,	M0070.grade
	,	M0050.grade_nm
	,	M0070.office_cd
	,	ISNULL(IIF(CAST(M0010.office_ab_nm as nvarchar(50)) = '',NUll,CAST(M0010.office_ab_nm as nvarchar(50))),M0010.office_nm)								AS office_nm
	,	S0010.user_id
	,	ISNULL(S0010.empinfo_authority_cd, 0)
	,	S5020.authority_nm
	FROM M0070
	LEFT JOIN M0060 ON (
	...
	LEFT JOIN S5020 ON (
		M0070.company_cd				=	S5020.company_cd
	AND	S0010.empinfo_authority_cd		=	S5020.authority_cd
	)
	LEFT JOIN S5020 AS S5020_LOGIN ON (
		M0070.company_cd		=	S5020_LOGIN.company_cd
	AND	@P_authority_cd			=	S5020_LOGIN.authority_cd
	)
	WHERE
		M0070.company_cd	=	@P_company_cd
	AND	M0070.del_datetime IS NULL

	AND ISNULL(S0010.empinfo_authority_typ,0) <> 5				-- add by viettd 2021/10/12
	AND (
		@P_employee_cd = ''
	...
	OR	(@P_authority_cd_screen <> 0 AND S0010.empinfo_authority_cd = @P_authority_cd_screen)
	)
	AND (
			@P_authority_typ	<> 3
		OR (
				@P_authority_typ	= 3 
				AND(
					(ISNULL(S5020_LOGIN.use_typ,0)	<> 1 OR ( ISNULL(S5020_LOGIN.use_typ,0)  = 1 AND ISNULL(M0040.arrange_order,0) > @arrange_order ))
				)
			)
	)
	...
	BEGIN
		...
		WHERE
			ISNULL(S0010.empinfo_authority_cd,0) <> 0
		AND S0010.del_datetime IS NULL
	END
	-- get permission
	INSERT INTO #TABLE_ORGANIZATION
	EXEC [dbo].SPC_REFER_ORGANIZATION_FND1 @P_json,@P_user_id,@P_company_cd,6
	--
	...
	BEGIN
		...
		END
		ELSE IF NOT (@P_authority_typ = 3 AND @w_setting_organization_cnt = 0 AND @w_organization_belong_person_typ = 0) -- edited by viettd 2021/06/03
		BEGIN
			DELETE D FROM #M0070 AS D
			FULL OUTER JOIN #TABLE_ORGANIZATION AS S ON (
				D.belong_cd1			=	S.organization_cd_1
			...
			WHERE 
				D.employee_cd IS NULL
			OR	S.organization_typ IS NULL
			AND @P_authority_typ  NOT IN(4,5)
		END
	END
	...
	--
	DROP TABLE #M0070, #TABLE_ORGANIZATION
