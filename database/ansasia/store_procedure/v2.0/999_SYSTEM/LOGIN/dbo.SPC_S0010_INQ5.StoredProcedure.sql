...
GO
-- +--TEST--+ S0010
-- EXEC SPC_S0010_INQ5 '{"contract_cd":"MC","user_id":"mcadmin","password":"mirai","sso_id":"mcadmin"}';
--****************************************************************************************
--*   											
--*  処理概要/process overview	:	LOGIN (SSO対応版)
--*  
--*  作成日/create date			:	2022/05/31						
--*　作成者/creater				:	vietdt							
--*   					
--*  更新日/update date			:	2023/07/20
--*　更新者/updater				:　	viettd
--*　更新内容/update content		:	upgrade 2.0
--****************************************************************************************
CREATE PROCEDURE [SPC_S0010_INQ5]
...
BEGIN
	...
	,	@ERR_TBL						ERRTABLE
	,	@order_by_min					INT					=	0
	,	@user_id						NVARCHAR(100)		=	ISNULL(JSON_VALUE(@P_json,'$.user_id'),'')
	,	@P_sso_id						NVARCHAR(255)		=	ISNULL(JSON_VALUE(@P_json,'$.sso_id'),'')		
	,	@password						NVARCHAR(40)		=	ISNULL(JSON_VALUE(@P_json,'$.password'),'')
	,	@P_contract_cd					NVARCHAR(20)		=	ISNULL(JSON_VALUE(@P_json,'$.contract_cd'),'')	
	,	@pass_remain					INT					=	0	
	...
	,	@failed_login_count				TINYINT				=	0		
	,	@w_user_is_rater_1				TINYINT				=	0	-- 0.IS NOT RATER_1 | 1.IS RATER_1 
	...
	AND del_datetime IS NULL
	--SET @w_setting_authority_typ ,@w_1on1_authority_typ , @w_multireview_authority_typ
	SELECT 
	...
	BEGIN
		SET @w_empinfo_authority_typ = 1
	END
	--			
	CREATE TABLE #SESSION_TBL (
		id								INT				IDENTITY(1,1) NOT NULL
	,	company_cd						SMALLINT
	,	company_nm						NVARCHAR(100)
	,	[user_id]						NVARCHAR(50)
	...
	,	authority_nm					NVARCHAR(20)
	,	[1on1_authority_nm]				NVARCHAR(20)
	,	multireview_authority_nm		NVARCHAR(20)
	...
	BEGIN TRY
		...
		BEGIN
			...
			,	S0010.employee_cd
			,	S0010.password
			-- evaluation_authority_typ tuong duong voi  authority_typ phien ban cu
