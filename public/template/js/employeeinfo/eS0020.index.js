...
function initialize() {
    try {
        ...
        _formatTooltip();
        // quangnd-tooltip
        let width = $('#table-organization').width();
        $('.resize_1').css({ width: width - 60 - 40 });
        for (let i = 2; i < 6; i++) {
            $('.resize_' + i).css({ width: width - 40 -(i-1)*40 });
        }
    } catch (e) {
        alert('initialize: ' + e.message);
    }
...
function getLeftContent(page, search) {
    try {
        ...
        $.ajax({
            type: 'POST',
            url: '/employeeinfo/es0020/leftcontent',
            dataType: 'html',
            loading: true,
...
function deleteData() {
    try {
        ...
        $.ajax({
            type: 'POST',
            url: '/employeeinfo/es0020/delete',
            dataType: 'json',
            loading: true,
            data: data,
            success: function (res) {
...
function getRightContent(authority_cd) {
    try {
        ...
        $.ajax({
            type: 'POST',
            url: '/employeeinfo/es0020/rightcontent',
            dataType: 'json',
            loading: false,
