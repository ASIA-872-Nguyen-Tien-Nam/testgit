...
this.title_report = "報告";
this.title_error = "エラー";
...
function initEvents() {
  try {
    $(document).on(
      "click",
      ".pagination li a.page-link:not(.pagging-disable)",
      function (e) {
        //alert(1);
        e.preventDefault();
  ...
  // click btn-show => show/hide column
  $(document).on("click", "#btn-show", function (event) {
    try {
      ...
      if (!$(this).hasClass("clicked")) {
        ...
        $(this).find("#btn_text").text(display);
      ...
      console.log("click btn-show: " + e.message);
    }
  });
}
...
function search(page, page_size) {
  try {
    ...
    $.ajax({
      type: "POST",
      url: "/employeeinfo/es0030/search",
      dataType: "html",
      loading: true,
      data: { data: obj },
      success: function (res) {
        ...
        if (btnText) {
          ...
          $("#btn-show").find("#btn_text").text(hide);
          ...
          $("#btn-show").find("#btn_text").text(display);
    ...
    console.log("search function: " + e.message);
  }
}
...
function deleteData() {
  try {
    ...
    $.ajax({
      type: "POST",
      url: "/employeeinfo/es0030/delete",
      dataType: "json",
      loading: true,
      data: JSON.stringify(data),
      success: function (res) {
