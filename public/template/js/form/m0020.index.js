...
function initEvents() {
	...
	$('#btn-change-org-name').on('click', function(e) {
		showPopup(
	...
	$('#btn-create-org').on('click', function(e) {
		//
	...
	$('#btn-Create-Division').on('click', function(e) {
		//
		jMessage(5, function(r) {
            var organization_cd_1 = $('#organization_cd_1').val();
            var organization_cd_2 = $('#organization_cd_2').val();
    ...
    $('#responsible_cd').on('change', function(e) {
        var $this = $(this);
        var data = {employee_cd: $this.val()};
        $.post('/master/m0020/checkemployee', data).then(res => {
            if (!res.exists) {
...
function makeLi(item, index, level, time) {
    ...
    a_props.txt = item.organization_nm;
    a_props.href = "#"+parent;
    a_props.typ = level;
    a_props.cd = item.organization_cd_1+'-'+item.organization_cd_2+'-'+item.organization_cd_3+'-'+item.organization_cd_4+'-'+item.organization_cd_5;

    var a = $('<a>', a_props);
    div_props["data-original-title"] = item.organization_nm;
    var div = $('<div>', div_props)
