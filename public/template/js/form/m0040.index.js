  ...
  try {
    ...
    $(document).on('click', '#btn-add-new', function(e) {
        jMessage(5,function(){
            $('#position_nm').focus();
            $('#position_cd').val('');
            $('#position_nm, #position_ab_nm,#arrange_order').val('');
...
function saveData() {
    try {
        ...
        var url = _customerUrl('/master/m0040/save');
        // send data to post
        $.ajax({
            type        :   'POST',
            url         :   url,
...
function deleteData() {
    try {
        var data = getData(_obj);
        var url = _customerUrl('/master/m0040/delete');
        // send data to post
        $.ajax({
            type        :   'POST',
            url         :   url,
...
function getLeftContent(page, search) {
    try {
        var url = _customerUrl('/master/m0040/leftcontent');
        // send data to post
        $.ajax({
            type        :   'POST',
            url         :   url,
...
function getRightContent(position_cd) {
    try {
        var url = _customerUrl('/master/m0040/rightcontent');
        // send data to post
        $.ajax({
            type        :   'POST',
            url         :   url,
