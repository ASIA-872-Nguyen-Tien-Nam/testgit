	...
	try {
		...
		$(document).on('click', '.link_ri2010', function (e) {
			try {
				...
				// 
				let list = [];
				$('.tab-content .active table tbody .tr_employee .link_ri2010').each(function () {
		...
		$(document).on('change', '#fiscal_year, #month, #times, #mygroup_cd', function (e) {
			try {
		...
		// change #organization_step1 ~ #organization_step3
		$(document).on('change', '#organization_step1, #organization_step2, #organization_step3', function (e) {
			try {
				__referDataAffterChangeMonth();
			} catch (e) {
				alert('change conditions: ' + e.message);
		...
		$(document).on('click', '#unapproved_only, #approved_show', function () {
			try {
				__referDataAffterChangeMonth();
			} catch (e) {
				alert('change conditions ' + e.message);
...
function __referDataAffterChangeMonth() {
	try {
		if (_validate()) {
			...
			var unapproved_only = 0;
			var approved_show = 0;
			if ($('#unapproved_only').prop('checked')) {
				unapproved_only = 1;
			}
			...
			data.unapproved_only = unapproved_only;
			data.approved_show = approved_show;
			// send data to post
			$.ajax({
				type: 'POST',
				url: '/weeklyreport/rdashboardapprover',
				dataType: 'html',
				loading: true,
				data: data,
		...
		console.log('__referDataAffterChangeMonth:' + e.message);
	}
}
				...
				switch (res['status']) {
					...
					case OK:
						var data = {
							...
							, 'from': 'rdashboardapprover'
						};
						data['screen_id'] = 'rdashboardapprover_ri2010';	// save key -> to cache
