...
function initEvents() {
	try {
		...
		$(document).on("click", "#btn-excel", function (e) {
			...
			if (_validate($("body"))) {
				excel();
				// $.downloadFileAjax('/weeklyreport/rq3010/export-excel', JSON.stringify(data));
			}
		});
	} catch (e) {
...
function excel() {
	try {
		...
		//
		list = [];
		$('#report_kind').closest('.multi-select-full').find('input[type=checkbox]').each(function () {
			if ($(this).prop('checked')) {
				list.push({
					'report_kind': $(this).val()
				});
			}
		});
		data.data_sql.list_report_kind = list;
		list = [];
...
function search() {
	try {
		...
		$.ajax({
			type: 'POST',
			url: '/weeklyreport/rq3010/search',
			dataType: 'html',
			loading: true,
