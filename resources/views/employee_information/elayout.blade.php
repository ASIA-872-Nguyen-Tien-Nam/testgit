...
<head>
    ...
    {!! public_url('template/css/employeeinfo/common/employee_information.css') !!}
            ...
            <div class="col-8" style="display:flex">
                @isset(session_data()->m0070->mypurpose_use_typ)
                    @if (session_data()->m0070->mypurpose_use_typ == 1)
                        ...
                        <button class="btn btn-outline-primary my_purpose_btn" mode="4"
        ...
        <div style="width:100%">
            <div class="col-12" style="padding: 0px">
                <div class="collapse navbar-collapse menu-common" id="navbarSupportedContent">
                    <div class="row" style="background: #31b5cc ; width:100%;margin:0 auto">
                        <div class="col-12">
                            <ul class="nav flex-column">
                                @if ($navbars[4])
                                    @foreach ($navbars[4] as $key => $row)
                                        <li class="nav-item">
                                            <a class="nav-link active" aria-current="page"
                                                href="{{ url($row->function_id) }}">
                                                {{ $row->function_nm }}
                                            </a>
                                        </li>
                                    @endforeach
                                @endif
                                <li class="nav-item">
                                    <a class="nav-link active" id="btn-change-password" style="color:white"
                                        aria-current="page"><span></span></i> {{ __('messages.change_password') }}</a>
                                ...
                                <li class="nav-item">
                                    <a class="nav-link active" id="btn-change-language"
                                        style="color:white; cursor:pointer" aria-current="page"><span></span></i>
                                        {{ __('messages.language_setting') }}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link active" to="#help"
                                        href="{{ env('GUIDE_SETTING_ADMIN_URL') }}"
                                        target="_blank">{{ __('messages.how_to_use_faq') }}<br>{{ __('messages.admin_use') }}</a>
                                </li>
                                @isset($navbars[4])
                                    @if (count($navbars[4]) == 1)
                                        <li class="nav-item">
                                            <a class="nav-link active" href="/basicsetting/sq9001"
                        ...
                        <div class="col-12" style="margin:0 auto;margin-top:20px;margin-bottom:20px">
                            <button class="btn btn-primary" style="background: #f7f7f7;margin:0 auto;width:100%">
                                <a class="nav-link" href="{{ url('logout') }}" style="color:black;padding:0px"><span><i
    ...
    <nav class="navbar navbar-expand-md fixed-top header-oneonone header header-xam navbar-pc">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            ...
            <ul class="navbar-nav right">
                <li class="nav-item" style="background-color: #31b5cc">
                    <a tabindex="-1" class="nav-link icon" to="#help"
                        href="{{ env('GUIDE_SETTING_ADMIN_URL', '') }}" target="_blank" style="color: #31b5cc;">
                        <span
                            style="color: white">{{ __('messages.how_to_use_faq') }}<br>{{ __('messages.admin_use') }}</span>
                    </a>
                </li>
                <li class="dropdown">
                    ...
                    @foreach ($navbars[4] as $key => $row)
                        @if ($key == 0)
                            <li class="parent-list">
                                <a tabindex="-1" id="home_url" href="{{ url($row->function_id) }}">
                                    <p>
                                        <img src={!! public_url('uploads/ver1.7/icon/icon-home.svg') !!}>
                                    </p>
                                    <div>{{ $row->function_nm }}</div>
                                </a>
                            <li>
                            @else
                            <li class="parent-list">
                                <a tabindex="-1" href="{{ url($row->function_id) }}">
                                    <p>
