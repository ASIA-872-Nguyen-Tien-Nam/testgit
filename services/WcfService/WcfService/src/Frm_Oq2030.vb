...
Public Class Frm_Oq2030
    Public Function FNC_EXL_Oq2030(
       ByVal sql As String,
       ...
       ByVal pathFile As String
        ...
        Try
            ...
            If Utl_Com.FNC_CNV_NUL(D_DAT.Tables(0).Rows(0).Item("language"), 0) = "en" Then
                D_EXL_TML = ConfigurationManager.AppSettings("FIL_TEM_EXL") & "\Oq2030_en.xlsx"
            Else
                D_EXL_TML = ConfigurationManager.AppSettings("FIL_TEM_EXL") & "\Oq2030.xlsx"
            End If
            ...
            Using D_EXL_BOK As New ExcelPackage(newFile)
                ...
                D_EXL_SHT1.Cells(2, 2).Value = Utl_Com.FNC_CNV_NUL(D_DAT.Tables(1).Rows(0).Item("group_cd_1on1_nm"), "")
                ...
                For i = 0 To D_ROW_CNT - 1
                    If i <> 0 Then
                        D_EXL_SHT1.Cells("A14:M14").Copy(D_EXL_SHT1.Cells("A" & (13 + i) & ":M" & (13 + i)))
                    End If
                    ...
                    D_EXL_SHT1.Cells(13 + i, 11).Value = Utl_Com.FNC_CNV_NUL(D_DAT.Tables(0).Rows(i).Item("month_no11"), "")
                    D_EXL_SHT1.Cells(13 + i, 12).Value = Utl_Com.FNC_CNV_NUL(D_DAT.Tables(0).Rows(i).Item("month_no12"), "")
                    D_EXL_SHT1.Cells(13 + i, 13).Value = Utl_Com.FNC_CNV_NUL(D_DAT.Tables(0).Rows(i).Item("annual_average"), "")
                Next
                If D_DAT.Tables(0).Rows(1).Item("annual_average") > 0 Or
                    D_DAT.Tables(0).Rows(3).Item("annual_average") > 0 Then
                    If D_DAT.Tables(0).Rows(1).Item("annual_average") = "" Or D_DAT.Tables(0).Rows(1).Item("annual_average") = 0 Then
                        D_EXL_SHT1.Cells(2, 13).Value = 0
                        D_EXL_SHT1.Cells(3, 13).Value = "0%"
                    End If
                    If D_DAT.Tables(0).Rows(3).Item("annual_average") = "" Or D_DAT.Tables(0).Rows(3).Item("annual_average") = 0 Then
                        D_EXL_SHT1.Cells(4, 13).Value = 0
                        D_EXL_SHT1.Cells(5, 13).Value = "0%"
                    End If
                End If
                '
                'D_EXL_SHT1.Cells("A" & (D_ROW_CNT) & ":M" & (D_ROW_CNT)).Style.Font.Bold = True
                D_EXL_SHT1.Cells("A18:M18").Style.Font.Bold = True
                '
                D_EXL_SHT1.Cells("A14:A15").Merge = True
                D_EXL_SHT1.Cells("A16:A17").Merge = True
ACTIVE:
                'Save file
                D_EXL_BOK.Save()
            End Using
        Catch ex As Exception
            Utl_ERR.WriteLogReport(ex.ToString(), "Oq2030")
            message = "FNC_EXL_Oq2030: " & ex.ToString()
            status = "201"
        Finally
            D_DAT.Clear()
            '
        End Try
EXIT_FUNCTION:
        FNC_EXL_Oq2030 = "{" + """" + "status" + """" + ":" + status + "," + """" + "filename" + """" + ":" + """" + fileName + """," + """" + "message" + """" + ":" + """" + message + """}"
        Exit Function
