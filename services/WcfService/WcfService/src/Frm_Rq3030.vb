...
Public Class Frm_Rq3030
    Public Function FNC_EXL_Rq3030(
       ByVal sql As String,
       ...
       ByVal pathFile As String
        ...
        Try
            ...
            If D_ROW_CNT = 0 Then
                status = "203"
                message = "FNC_EXL_Rq3030: Data is empty"
                GoTo EXIT_FUNCTION
            End If
            '
            If Utl_Com.FNC_CNV_NUL(D_DAT.Tables(1).Rows(0).Item("language"), "") = "en" Then
                D_EXL_TML = ConfigurationManager.AppSettings("FIL_TEM_EXL") & "\Rq3030_en.xlsx"
            Else
                D_EXL_TML = ConfigurationManager.AppSettings("FIL_TEM_EXL") & "\Rq3030.xlsx"
            End If
            ...
            Using D_EXL_BOK As New ExcelPackage(newFile)
                ...
                D_EXL_SHT1.Cells(12, 2).Value = Utl_Com.FNC_CNV_NUL(D_DAT.Tables(2).Rows(0).Item("field_3030_nm"), "")
                ' fill data
                'HEARDER
...
ACTIVE:
        ...
        Catch ex As Exception
            ...
            message = "FNC_EXL_Rq3030: " & ex.ToString()
            status = "201"
        Finally
            D_DAT.Clear()
            '
        End Try
EXIT_FUNCTION:
        FNC_EXL_Rq3030 = "{" + """" + "status" + """" + ":" + status + "," + """" + "filename" + """" + ":" + """" + fileName + """," + """" + "message" + """" + ":" + """" + message + """}"
        Exit Function
